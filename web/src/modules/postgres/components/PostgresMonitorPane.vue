...
const instance = ref<PostgresMetaInstanceOverviewResult | null>(null)
const sessions = ref<PostgresActivitySession[]>([])
const locks = ref<PostgresLockInfo[]>([])
const blockingEdges = ref<PostgresLockBlockingEdge[]>([])
...
const detailSession = ref<PostgresActivitySession | null>(null)
...
const variables = ref<PostgresServerKVItem[]>([])
...
const tabItems = computed((): RsTabItem[] => [
  { value: 'instance', label: t('modules.postgres.monitor.tabInstance') },
  {
    value: 'sessions',
    label: t('modules.postgres.monitor.tabSessions'),
    badge: filteredSessions.value.length || undefined,
  },
  {
    value: 'locks',
    label: t('modules.postgres.monitor.tabLocks'),
...
const kvColumns = computed((): RsTableColumn<KVRow>[] => [
  { key: 'name', title: t('modules.postgres.monitor.kvName'), minWidth: 220, ellipsis: true },
  { key: 'value', title: t('modules.postgres.monitor.kvValue'), minWidth: 280, ellipsis: true },
])
...
const intervalOptions = computed((): RsSelectOptions => [
  { value: '0', label: t('modules.postgres.monitor.autoOff') },
  { value: '5', label: t('modules.postgres.monitor.autoSecs', { n: 5 }) },
  { value: '10', label: t('modules.postgres.monitor.autoSecs', { n: 10 }) },
  { value: '30', label: t('modules.postgres.monitor.autoSecs', { n: 30 }) },
])

const filterOptions = computed((): RsSelectOptions => [
  { value: 'all', label: t('modules.postgres.monitor.filterAll') },
  { value: 'active', label: t('modules.postgres.monitor.filterActive') },
  { value: 'waiting', label: t('modules.postgres.monitor.filterWaiting') },
  { value: 'idle', label: t('modules.postgres.monitor.filterIdle') },
])
...
const sessionCols = computed((): RsTableColumn<Row>[] => [
  ...
  { key: 'userName', title: t('modules.postgres.monitor.colUser'), minWidth: 72, ellipsis: true },
  { key: 'database', title: t('modules.postgres.monitor.colDatabase'), minWidth: 88, ellipsis: true },
  {
    key: 'applicationName',
    title: t('modules.postgres.monitor.colApp'),
    minWidth: 100,
    ellipsis: true,
  },
  { key: 'state', title: t('modules.postgres.monitor.colState'), minWidth: 72 },
  { key: 'durationMs', title: t('modules.postgres.monitor.colDuration'), minWidth: 88, align: 'right' },
  {
    key: 'clientAddr',
    title: t('modules.postgres.monitor.colClientAddr'),
    minWidth: 120,
    ellipsis: true,
  },
  {
    key: 'backendType',
    title: t('modules.postgres.monitor.colBackendType'),
    minWidth: 88,
    ellipsis: true,
  },
  { key: 'waitEvent', title: t('modules.postgres.monitor.colWait'), minWidth: 100, ellipsis: true },
  { key: 'query', title: t('modules.postgres.monitor.colQuery'), minWidth: 200, ellipsis: true },
])
...
const lockCols = computed((): RsTableColumn<Row>[] => [
  ...
  {
    key: 'blockedByPid',
    title: t('modules.postgres.monitor.colBlockedBy'),
    minWidth: 88,
    align: 'right',
  },
  {
    key: 'blockChain',
    title: t('modules.postgres.monitor.colBlockChain'),
    minWidth: 140,
    ellipsis: true,
  },
  { key: 'lockType', title: t('modules.postgres.monitor.colLockType'), minWidth: 96 },
  { key: 'mode', title: t('modules.postgres.monitor.colMode'), minWidth: 100, ellipsis: true },
  { key: 'granted', title: t('modules.postgres.monitor.colGranted'), minWidth: 72 },
  { key: 'relation', title: t('modules.postgres.monitor.colRelation'), minWidth: 140, ellipsis: true },
  { key: 'database', title: t('modules.postgres.monitor.colDatabase'), minWidth: 100, ellipsis: true },
])

const sessionByPid = computed(() => {
  const map = new Map<number, PostgresActivitySession>()
...
function formatClient(s: PostgresActivitySession): string {
...
function canCancelSession(s: PostgresActivitySession | null | undefined): boolean {
  if (!s) return false
  const state = (s.state ?? '').toLowerCase()
...
function stopAutoRefreshForGone(): void {
  ...
  clearRefreshTimer()
  toast.warning(t('modules.postgres.monitor.sessionGone'))
}
...
const lockRows = computed((): Row[] =>
  filteredLocks.value.map((l, i) => ({
    ...
    mode: l.mode ?? '',
    granted: l.granted
      ? t('modules.postgres.session.colTipYes')
      : t('modules.postgres.session.colTipNo'),
    relation: l.relation ?? '',
    database: l.database ?? '',
  })),
)
...
const detailRows = computed(() => {
  ...
  return [
    ...
    { key: 'userName', label: t('modules.postgres.monitor.colUser'), value: s.userName || '—' },
    { key: 'database', label: t('modules.postgres.monitor.colDatabase'), value: s.database || '—' },
    {
      key: 'applicationName',
      label: t('modules.postgres.monitor.colApp'),
      value: s.applicationName || '—',
    },
    { key: 'state', label: t('modules.postgres.monitor.colState'), value: s.state || '—' },
    {
      key: 'durationMs',
      label: t('modules.postgres.monitor.colDuration'),
      value: formatDurationMs(s.durationMs) || '—',
    },
    {
      key: 'queryStart',
      label: t('modules.postgres.monitor.colQueryStart'),
      value: s.queryStart || '—',
    },
    {
      key: 'xactStart',
      label: t('modules.postgres.monitor.colXactStart'),
      value: s.xactStart || '—',
    },
    {
      key: 'waitEventType',
      label: t('modules.postgres.monitor.colWaitType'),
      value: s.waitEventType || '—',
    },
    { key: 'waitEvent', label: t('modules.postgres.monitor.colWait'), value: s.waitEvent || '—' },
    {
      key: 'clientAddr',
      label: t('modules.postgres.monitor.colClientAddr'),
      value: formatClient(s) || '—',
    },
    {
      key: 'backendType',
      label: t('modules.postgres.monitor.colBackendType'),
      value: s.backendType || '—',
    },
    {
      key: 'sessionId',
      label: t('modules.postgres.monitor.colSessionId'),
      value: s.sessionId || '—',
    },
    {
      key: 'queryId',
      label: t('modules.postgres.monitor.colQueryId'),
      value: s.queryId || '—',
    },
    { key: 'query', label: t('modules.postgres.monitor.colQuery'), value: s.query || '—' },
  ]
})
...
const confirmTitle = computed(() =>
  pendingAction.value === 'terminate'
    ? t('modules.postgres.monitor.terminateTitle')
    : t('modules.postgres.monitor.cancelTitle'),
)

const confirmDesc = computed(() =>
  t(
    pendingAction.value === 'terminate'
      ? 'modules.postgres.monitor.terminateDesc'
      : 'modules.postgres.monitor.cancelDesc',
    { pid: pendingPid.value ?? '' },
  ),
)

function errMessage(e: unknown): string {
  return e instanceof Error ? e.message : t('modules.postgres.monitor.loadError')
}
...
function openSessionDetail(row: Row | null): void {
  ...
  if (!session) {
    toast.error(t('modules.postgres.monitor.sessionNotFound'))
    return
  }
...
function sessionContextMenuItems(row: Row | null): RsContextMenuItem[] {
  ...
  return [
    {
      key: 'session-detail',
      label: t('modules.postgres.monitor.viewSessionDetail'),
      icon: 'info',
    },
    { key: 'sep-ops', label: '', separator: true },
    {
      key: 'session-cancel',
      label: t('modules.postgres.monitor.cancelQuery'),
      icon: 'circle-x',
      disabled: !canCancelSession(session),
    },
    {
      key: 'session-terminate',
      label: t('modules.postgres.monitor.terminateSession'),
      icon: 'unplug',
      danger: true,
    },
  ]
...
function lockContextMenuItems(row: Row | null): RsContextMenuItem[] {
  ...
  const items: RsContextMenuItem[] = [
    {
      key: 'lock-jump-pid',
      label: `${t('modules.postgres.monitor.jumpToSession')} (PID ${row.pid})`,
      icon: 'arrow-right',
    },
  ]
  if (Number.isFinite(blockedBy) && blockedBy > 0) {
    items.push({
      ...
      label: `${t('modules.postgres.monitor.jumpToSession')} (${t('modules.postgres.monitor.colBlockedBy')} ${blockedBy})`,
      icon: 'arrow-right',
    })
  }
  items.push(
    { key: 'sep-lock-ops', label: '', separator: true },
    {
      ...
      label: `${t('modules.postgres.monitor.terminateSession')} (PID ${pid})`,
      icon: 'unplug',
      danger: true,
    },
  )
  if (Number.isFinite(blockedBy) && blockedBy > 0) {
    items.push({
      ...
      label: `${t('modules.postgres.monitor.terminateSession')} (${t('modules.postgres.monitor.colBlockedBy')} ${blockedBy})`,
      icon: 'unplug',
      danger: true,
    })
  }
...
async function confirmBackendAction(): Promise<void> {
  ...
  try {
    const result =
      action === 'terminate'
        ? await postgresApi.metaBackendTerminate({ ...apiBase(), pid })
        : await postgresApi.metaBackendCancel({ ...apiBase(), pid })
    if (result.success) {
      toast.success(
        action === 'terminate'
          ? t('modules.postgres.monitor.terminateOk', { pid })
          : t('modules.postgres.monitor.cancelOk', { pid }),
      )
    } else {
      toast.warning(t('modules.postgres.monitor.actionNoEffect', { pid }))
    }
...
async function loadVariables(opts?: { silent?: boolean }): Promise<void> {
  ...
  try {
    const result = await postgresApi.metaServerVariables(apiBase())
...
async function loadStatus(opts?: { silent?: boolean }): Promise<void> {
  ...
  if (!opts?.silent) kvBusy.value = true
  try {
    const result = await postgresApi.metaServerStatus(apiBase())
...
async function loadCore(opts?: { silent?: boolean }): Promise<void> {
  ...
  try {
    ...
    const [instRes, actRes, locksRes] = await Promise.allSettled([
      postgresApi.metaInstanceOverview(base),
      postgresApi.metaActivity(base),
      postgresApi.metaLocks(base),
    ])
  ...
  <div class="nm-vast-monitor">
      ...
      <template #left>
        <div class="nm-vast-monitor__identity" :title="sessionLabel || undefined">
          ...
          <span class="nm-vast-monitor__feature">
            ...
            {{ t('modules.postgres.session.tabMonitor') }}
          </span>
        </div>
        <RsTabs
          v-model="section"
    ...
    >
      ...
      <RsInput
        ...
        :placeholder="t('modules.postgres.monitor.filterPlaceholder')"
      />
    ...
    >
      ...
      >
        {{ t('modules.postgres.monitor.lockWaitingOnly') }}
      </RsButton>
      <span class="nm-vast-monitor__filter-meta">
    ...
    >
      <RsInput
        ...
        :placeholder="t('modules.postgres.monitor.kvFilterPlaceholder')"
      />
      <span class="nm-vast-monitor__filter-meta">
        {{ filteredKV.length }} /
        {{ section === 'variables' ? variables.length : statusItems.length }}
        <template v-if="kvTruncated">
          · {{ t('modules.postgres.monitor.kvTruncated') }}
        </template>
      </span>
    </div>
    ...
    >
      ...
      <RsEmpty
        v-else-if="!scopeOk"
        icon="activity"
        :description="t('modules.postgres.monitor.needConnection')"
      />
      ...
      <template v-else-if="section === 'instance'">
        ...
        <RsEmpty
          v-else-if="!loading"
          icon="activity"
          :description="t('modules.postgres.monitor.loadError')"
        />
      </template>
      ...
      <template v-else-if="section === 'sessions'">
        ...
        <RsEmpty
          ...
          :description="
            sessions.length === 0
              ? t('modules.postgres.monitor.sessionsEmpty')
              : t('modules.postgres.monitor.filterEmpty')
          "
        />
        <RsTable
          v-else
      ...
      <template v-else-if="section === 'locks'">
        ...
        <RsEmpty
          ...
          :description="
            locks.length === 0
              ? t('modules.postgres.monitor.locksEmpty')
              : t('modules.postgres.monitor.filterEmpty')
          "
        />
        <RsTable
        ...
        <RsEmpty
          ...
          :description="
            (section === 'variables' ? variables.length : statusItems.length) === 0
              ? t('modules.postgres.monitor.kvEmpty')
              : t('modules.postgres.monitor.filterEmpty')
          "
        />
        <RsTable
    ...
    <RsDialog
      ...
      :title="t('modules.postgres.monitor.sessionDetailTitle')"
      width="md"
      layout="form"
      :modal="false"
      :show-overlay="false"
    ...
    >
      ...
      <template #footer>
        ...
        >
          {{ t('modules.postgres.monitor.cancelQuery') }}
        </RsButton>
        <RsButton
          variant="ghost"
          size="sm"
        ...
        >
          {{ t('modules.postgres.monitor.terminateSession') }}
        </RsButton>
