...
/**
 * Postgres 表设计器：挂载公共 TableDesignShell，方言负责草稿 / RPC / 类型目录。
...
import {
  ...
  toForeignKeyDrafts,
  toIndexDrafts,
} from '@/modules/postgres/utils/table-design-ops'
import {
  dataTypeParamKind,
  ...
  parseColumnList,
  syncColumnDataType,
  POSTGRES_BASE_TYPE_OPTIONS,
  POSTGRES_INDEX_METHODS,
...
const snapshot = ref<PostgresColumnInfo[]>([])
const pkSnapshot = ref<string[]>([])
const constraints = ref<PostgresConstraintInfo[]>([])
...
const typeBaseSelectOptions = POSTGRES_BASE_TYPE_OPTIONS.map((o) => ({
  value: o.base,
  label: o.base,
}))

const indexMethodSelectOptions = POSTGRES_INDEX_METHODS.map((v) => ({ value: v, label: v }))
...
const columnColumns = computed((): RsTableColumn<ColRow>[] => {
  const cols: RsTableColumn<ColRow>[] = [
    { key: 'name', title: t('modules.postgres.design.colName'), minWidth: 110, editable: true },
    {
      key: 'typeBase',
      title: t('modules.postgres.design.colType'),
      minWidth: 120,
      editable: true,
      valueType: 'select',
      editorOptions: {
    ...
    {
      key: 'typeLength',
      title: t('modules.postgres.design.colLength'),
      width: 72,
      align: 'center',
    ...
    {
      key: 'typeScale',
      title: t('modules.postgres.design.colScale'),
      width: 72,
      align: 'center',
    ...
    {
      key: 'primaryKey',
      title: t('modules.postgres.design.colPk'),
      width: 52,
      align: 'center',
      editable: true,
      valueType: 'boolean',
    ...
    {
      key: 'nullable',
      title: t('modules.postgres.design.colNullable'),
      width: 52,
      align: 'center',
      editable: true,
      valueType: 'boolean',
    ...
    {
      key: 'defaultExpr',
      title: t('modules.postgres.design.colDefault'),
      minWidth: 120,
      editable: true,
      ellipsis: true,
    },
    {
      key: 'comment',
      title: t('modules.postgres.design.colComment'),
      minWidth: 100,
      editable: true,
      ellipsis: true,
    },
  ...
  if (!modeCreate.value) {
    cols.push({
      key: 'status',
      title: t('modules.postgres.design.colStatus'),
      width: 72,
      align: 'center',
    })
  }
...
const indexColumns = computed((): RsTableColumn<IdxRow>[] => {
  ...
  const cols: RsTableColumn<IdxRow>[] = [
    {
      key: 'name',
      title: t('modules.postgres.design.idxName'),
      minWidth: 110,
      editable: (row) => !row.primary,
    },
    {
      key: 'kindLabel',
      title: t('modules.postgres.design.idxKind'),
      width: 88,
      editable: false,
    },
    {
      key: 'columnsText',
      title: t('modules.postgres.design.idxColumns'),
      minWidth: 160,
      editable: true,
      valueType: 'select',
      headerTip: t('modules.postgres.design.idxColumnsTip'),
      editorOptions: {
        options: colOptions,
        multiple: true,
        searchable: true,
    ...
    {
      key: 'method',
      title: t('modules.postgres.design.idxMethod'),
      width: 88,
      editable: (row) => !row.primary,
      valueType: 'select',
    ...
    {
      key: 'unique',
      title: t('modules.postgres.design.idxUnique'),
      width: 64,
      align: 'center',
      editable: (row) => !row.primary,
      valueType: 'boolean',
...
const fkColumns = computed((): RsTableColumn<FkRow>[] => {
  ...
  const cols: RsTableColumn<FkRow>[] = [
    { key: 'name', title: t('modules.postgres.design.fkName'), minWidth: 100, editable: true },
    {
      key: 'columnsText',
      title: t('modules.postgres.design.fkColumns'),
      minWidth: 120,
      editable: true,
      valueType: 'select',
      headerTip: t('modules.postgres.design.fkColumnsTip'),
      editorOptions: {
        options: colOptions,
        multiple: true,
        searchable: true,
    ...
    {
      key: 'refSchema',
      title: t('modules.postgres.design.fkRefSchema'),
      width: 100,
      editable: true,
      valueType: 'select',
      headerTip: t('modules.postgres.design.fkRefSchemaTip'),
      editorOptions: {
        options: schemaOpts,
        searchable: true,
        creatable: true,
    ...
    {
      key: 'refTable',
      title: t('modules.postgres.design.fkRefTable'),
      minWidth: 110,
      editable: true,
      valueType: 'select',
      headerTip: t('modules.postgres.design.fkRefTableTip'),
      editorOptions: {
        options: (row) => {
          const sch =
    ...
    {
      key: 'refColumnsText',
      title: t('modules.postgres.design.fkRefColumns'),
      minWidth: 120,
      editable: true,
      valueType: 'select',
      headerTip: t('modules.postgres.design.fkRefColumnsTip'),
      editorOptions: {
        options: (row) => {
    ...
    {
      key: 'onDelete',
      title: t('modules.postgres.design.fkOnDelete'),
      minWidth: 110,
      editable: true,
      valueType: 'select',
    ...
    {
      key: 'onUpdate',
      title: t('modules.postgres.design.fkOnUpdate'),
      minWidth: 110,
      editable: true,
      valueType: 'select',
      editorOptions: { options: fkActionSelectOptions },
...
const checkColumns = computed((): RsTableColumn<ChkRow>[] => {
  const cols: RsTableColumn<ChkRow>[] = [
    { key: 'name', title: t('modules.postgres.design.chkName'), minWidth: 110, editable: true },
    {
      key: 'expression',
      title: t('modules.postgres.design.chkExpression'),
      minWidth: 220,
      editable: true,
      ellipsis: true,
    },
  ...
  rows.value
    ...
    .map((r) => ({
      ...
      status: !r.originalName
        ? t('modules.postgres.design.statusNew')
        : r.name !== r.originalName
          ? t('modules.postgres.design.statusRename')
          : t('modules.postgres.design.statusEdit'),
    })),
)

function indexStatus(r: DesignIndexDraft): string {
  if (r.primary) return t('modules.postgres.design.idxPrimary')
  if (!r.originalName) return t('modules.postgres.design.statusNew')
  const changed =
    r.name !== r.snapName ||
  ...
  return changed
    ? t('modules.postgres.design.statusEdit')
    : t('modules.postgres.design.statusUnchanged')
}
...
const displayIndexes = computed((): IdxRow[] =>
  indexes.value
    ...
    .map((r) => ({
      ...r,
      kindLabel: r.primary
        ? t('modules.postgres.design.idxKindPrimary')
        : r.unique
          ? t('modules.postgres.design.idxKindUnique')
          : t('modules.postgres.design.idxKindNormal'),
...
const shellLabels = computed<TableDesignShellLabels>(() => ({
  reload: t('modules.postgres.design.reload'),
  preview: t('modules.postgres.design.preview'),
  create: t('modules.postgres.design.createTable'),
  apply: t('modules.postgres.design.apply'),
  previewTitle: t('modules.postgres.design.previewTitle'),
  selectRow: t('modules.postgres.design.selectRow'),
  copyPreview: t('modules.postgres.design.copyPreview'),
  moveUp: t('modules.postgres.design.moveUp'),
  moveDown: t('modules.postgres.design.moveDown'),
  add: t('modules.postgres.design.addColumn'),
}))

const title = computed(() =>
  modeCreate.value
    ? t('modules.postgres.design.createTitle')
    : t('modules.postgres.design.alterTitle', { name: tableName.value }),
)

const sections = computed<TableDesignSectionItem[]>(() => [
  {
    id: 'columns',
    label: t('modules.postgres.design.tabColumns'),
    count: displayColumns.value.length,
  },
  {
    id: 'indexes',
    label: t('modules.postgres.design.tabIndexes'),
    count: displayIndexes.value.length,
  },
  {
    id: 'foreignKeys',
    label: t('modules.postgres.design.tabForeignKeys'),
    count: displayForeignKeys.value.length,
  },
  {
    id: 'checks',
    label: t('modules.postgres.design.tabChecks'),
...
const addButtonLabel = computed(() => {
  if (activeSection.value === 'indexes') return t('modules.postgres.design.addIndex')
  if (activeSection.value === 'foreignKeys') return t('modules.postgres.design.addForeignKey')
  if (activeSection.value === 'checks') return t('modules.postgres.design.addCheck')
  return t('modules.postgres.design.addColumn')
})
  ...
  loadDesignInflight = (async () => {
    ...
    try {
      const [cols, pk, idx, fks, cons] = await Promise.all([
        ...
        postgresApi.metaIndexes({
          sessionId: props.sessionId,
          database: props.database,
          schema: props.schema,
          table: name,
        }),
        postgresApi.metaForeignKeys({
          sessionId: props.sessionId,
          database: props.database,
          schema: props.schema,
          table: name,
        }),
        postgresApi.metaConstraints({
          sessionId: props.sessionId,
          database: props.database,
          schema: props.schema,
          table: name,
      ...
      toast.error(e instanceof Error ? e.message : t('modules.postgres.design.loadError'))
    } finally {
      loading.value = false
    }
  })().finally(() => {
...
function formatPreviewSqls(sqls: string[]): string[] {
  return sqls.map((s) => {
    ...
    if (!raw) return raw
    try {
      return formatSql(raw, { dialect: 'postgresql' })
    } catch {
      return raw
    }
  })
...
async function loadPreviewSql(): Promise<boolean> {
  ...
  try {
    if (modeCreate.value) {
      ...
      if (columns.length === 0) {
        toast.info(t('modules.postgres.design.needColumns'))
        return false
      }
      const result = await postgresApi.ddlCreateTablePreview({
    ...
    const ops = alterOps()
    if (ops.length === 0) {
      previewSqls.value = [`-- ${t('modules.postgres.design.noChanges')}`]
      return true
    }
    const result = await postgresApi.ddlDesignPreview({
      sessionId: props.sessionId ?? undefined,
      profileId: props.profileId,
      database: props.database,
      schema: props.schema,
    ...
    toast.error(e instanceof Error ? e.message : t('modules.postgres.design.previewError'))
    return false
  }
}
...
async function copyPreviewSql(): Promise<void> {
  ...
  try {
    ...
    toast.success(t('modules.postgres.design.copyPreviewOk'))
  } catch {
    toast.error(t('modules.postgres.design.copyPreviewFailed'))
  }
}
...
function switchToAlterAfterCreate(name: string): void {
  ...
  if (!tab) return
  const designLabel = t('modules.postgres.session.tabDesign')
  const base =
...
async function apply(): Promise<void> {
  ...
  try {
    if (modeCreate.value) {
      ...
      const result = await postgresApi.ddlCreateTableApply({
        sessionId: props.sessionId,
        profileId: props.profileId,
        database: props.database,
        schema: props.schema,
      ...
      previewSqls.value = formatPreviewSqls(result.sql)
      toast.success(t('modules.postgres.design.createDone'))
      showPreview.value = false
    ...
    const ops = alterOps()
    if (ops.length === 0) {
      toast.info(t('modules.postgres.design.noChanges'))
      return
    }
    const result = await postgresApi.ddlDesignApply({
      sessionId: props.sessionId,
      database: props.database,
      schema: props.schema,
      name: tableName.value,
    ...
    previewSqls.value = formatPreviewSqls(result.sql)
    toast.success(t('modules.postgres.design.applyDone'))
    await loadDesign()
  } catch (e) {
    toast.error(
      e instanceof Error
        ? e.message
        : modeCreate.value
          ? t('modules.postgres.design.createError')
          : t('modules.postgres.design.applyError'),
    )
  } finally {
    saving.value = false
  }
  ...
  >
    <template #preview>
      <TableDesignPreviewPopover
        ...
        :empty-label="t('modules.postgres.design.noChanges')"
    ...
    <template #meta>
      <div class="nm-postgres-design__meta-row">
        <label class="nm-postgres-design__meta-label">{{ t('modules.postgres.design.tableName') }}</label>
        <RsInput
          v-if="modeCreate"
          v-model="draftTableName"
          size="sm"
          :placeholder="t('modules.postgres.design.tableNamePh')"
        />
        <span v-else class="nm-postgres-design__meta-readonly">{{ tableName }}</span>
      </div>
      <div class="nm-postgres-design__meta-row nm-postgres-design__meta-row--full">
        <label class="nm-postgres-design__meta-label">{{ t('modules.postgres.design.tableComment') }}</label>
        <RsInput
          v-model="tableComment"
          size="sm"
          :placeholder="t('modules.postgres.design.tableCommentPh')"
        />
      </div>
    </template>
    ...
    <template #list>
      <RsEmpty
        ...
        icon-radius="none"
        icon="table"
        :description="t('modules.postgres.structure.needTable')"
      />
      <template v-else-if="activeSection === 'columns'">
        <RsTable
          class="nm-postgres-design__grid"
            ...
            (row) =>
              row
                ? [
                    {
                      key: 'remove',
                      label: t('modules.postgres.design.removeColumn'),
                      icon: 'trash-2',
                      danger: true,
                    },
                  ]
                ...
                ? [
                    {
                      ...
                      label: row.originalName
                        ? t('modules.postgres.design.dropIndex')
                        : t('modules.postgres.design.removeIndex'),
                      icon: 'trash-2',
                      danger: true,
                    },
                  ]
                ...
                ? [
                    {
                      key: 'remove',
                      label: row.originalName
                        ? t('modules.postgres.design.dropForeignKey')
                        : t('modules.postgres.design.removeForeignKey'),
                      icon: 'trash-2',
                      danger: true,
                    },
                  ]
                ...
                ? [
                    {
                      key: 'remove',
                      label: row.originalName
                        ? t('modules.postgres.design.dropCheck')
                        : t('modules.postgres.design.removeCheck'),
                      icon: 'trash-2',
                      danger: true,
                    },
                  ]
    ...
    <template #editor>
      <template v-if="activeSection === 'columns' && editingCol">
        <div class="nm-postgres-design__editor-title">{{ t('modules.postgres.design.editColumn') }}</div>
        <div class="nm-postgres-design__form">
          <div class="nm-postgres-design__field">
            <label>{{ t('modules.postgres.design.colName') }}</label>
            <RsInput
          ...
          <div class="nm-postgres-design__field">
            <label>{{ t('modules.postgres.design.colType') }}</label>
            <RsSelect
          ...
          <div class="nm-postgres-design__field">
            <label>{{ t('modules.postgres.design.colDefault') }}</label>
            <RsInput
              :model-value="editingCol.defaultExpr"
              size="sm"
              :placeholder="t('modules.postgres.design.colDefaultPh')"
              @update:model-value="
          ...
          <div class="nm-postgres-design__field">
            <label>{{ t('modules.postgres.design.colComment') }}</label>
            <RsInput
              :model-value="editingCol.comment"
              size="sm"
              :placeholder="t('modules.postgres.design.colCommentPh')"
              @update:model-value="
                updateColSideField(editingCol!.__rowKey, 'comment', String($event ?? ''))
              "
            />
          </div>
          <p class="nm-postgres-design__hint">{{ t('modules.postgres.design.gridEditHint') }}</p>
        </div>
      </template>
      <template v-else-if="activeSection === 'indexes' && editingIdx">
        <div class="nm-postgres-design__editor-title">{{ t('modules.postgres.design.editIndex') }}</div>
        <div class="nm-postgres-design__form">
          <div class="nm-postgres-design__field">
            <label>{{ t('modules.postgres.design.idxName') }}</label>
            <RsInput
              :model-value="editingIdx.name"
              size="sm"
              :disabled="editingIdx.primary"
              :placeholder="t('modules.postgres.design.idxNamePh')"
              @update:model-value="
          ...
          <div class="nm-postgres-design__field">
            <label>{{ t('modules.postgres.design.idxMethod') }}</label>
            <RsSelect
          ...
          <div class="nm-postgres-design__field">
            <label>{{ t('modules.postgres.design.idxWhere') }}</label>
            <RsInput
      ...
      <template v-else-if="activeSection === 'foreignKeys' && editingFk">
        <div class="nm-postgres-design__editor-title">{{ t('modules.postgres.design.editForeignKey') }}</div>
        <div class="nm-postgres-design__form">
          <div class="nm-postgres-design__field">
            <label>{{ t('modules.postgres.design.fkRefSchema') }}</label>
            <RsSelect
          ...
          <div class="nm-postgres-design__field">
            <label>{{ t('modules.postgres.design.fkOnDelete') }}</label>
            <RsSelect
          ...
          <div class="nm-postgres-design__field">
            <label>{{ t('modules.postgres.design.fkOnUpdate') }}</label>
            <RsSelect
      ...
      <template v-else-if="activeSection === 'checks' && editingChk">
        <div class="nm-postgres-design__editor-title">{{ t('modules.postgres.design.editCheck') }}</div>
        <div class="nm-postgres-design__form">
          <div class="nm-postgres-design__field">
            <label>{{ t('modules.postgres.design.chkName') }}</label>
            <RsInput
          ...
          <div class="nm-postgres-design__field">
            <label>{{ t('modules.postgres.design.chkExpression') }}</label>
            <RsInput
              :model-value="editingChk.expression"
              size="sm"
              :placeholder="t('modules.postgres.design.chkExpressionPh')"
              @update:model-value="
                checks = checks.map((c) =>
      ...
      <div v-else class="nm-postgres-design__editor-empty">
        <RsEmpty
          fill
          radius="none"
          icon-radius="none"
          :description="t('modules.postgres.design.selectRow')"
        />
      </div>
    </template>
  </TableDesignShell>
...
<style scoped>
.nm-postgres-design__meta-row {
  display: flex;
  align-items: center;
  gap: 6px;
}
.nm-postgres-design__meta-row--full {
  flex: 1;
  min-width: 240px;
}
.nm-postgres-design__meta-label {
  font-size: 12px;
  color: var(--rs-fg-muted);
  white-space: nowrap;
  min-width: 60px;
}
.nm-postgres-design__meta-readonly {
  font-size: 12px;
  font-weight: 500;
  min-width: 80px;
}
.nm-postgres-design__grid {
  flex: 1;
  min-height: 0;
}
.nm-postgres-design__editor-title {
  font-size: 11px;
  font-weight: 600;
  color: var(--rs-fg-muted);
...
.nm-postgres-design__editor-empty {
  flex: 1;
  display: flex;
  align-items: center;
  justify-content: center;
}
.nm-postgres-design__form {
  display: flex;
  flex-direction: column;
  gap: 8px;
}
.nm-postgres-design__field {
  display: flex;
  flex-direction: column;
  gap: 3px;
}
.nm-postgres-design__field label {
  font-size: 11px;
  color: var(--rs-fg-muted);
}
.nm-postgres-design__hint {
  margin: 8px 0 0;
  font-size: 11px;
  color: var(--rs-fg-muted);
  line-height: 1.4;
