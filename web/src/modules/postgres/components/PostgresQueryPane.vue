...
const {
  ...
  commitTx,
  rollbackTx,
} = usePostgresQueryPane(props)
</script>

<template>
  <SqlQueryShell
    v-model:history-open="historyOpen"
    class="nm-postgres-query"
  ...
  >
    ...
    <template #editor>
      <RsMonacoEditor
        ...
        height="100%"
        class="nm-postgres-query__editor"
        :options="{
      ...
      />
      <div v-else class="nm-postgres-query__editor-boot">
        <RsLoading size="sm" />
      </div>
    </template>
...
.nm-postgres-query__editor-boot {
  display: flex;
  flex: 1;
  align-items: center;
