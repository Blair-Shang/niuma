...
export async function loadCategoryChildren(
  ...
  if (category === 'sequences') {
    const result = await postgresApi.treeSequences({
      profileId: conn.profileId,
      database,
      schema,
      filter: nameFilter,
  ...
  const result = await postgresApi.treeRoutines({
    profileId: conn.profileId,
    database,
    schema,
    kinds,
  ...
  const nodes: ConnResourceDescriptor[] = result.routines.map((rtn) => {
    ...
    return {
      path: { segments },
      label: rtn.args?.trim() ? `${rtn.name}(${rtn.args})` : rtn.name,
...
export async function loadSchemaCategories(
  ...
  let counts: Awaited<ReturnType<typeof postgresApi.treeCategoryCounts>> | null = null
  try {
    counts = await postgresApi.treeCategoryCounts({
      profileId: conn.profileId,
      database,
      schema,
    })
  } catch (err) {
    console.warn('[postgres] tree.categoryCounts failed', err)
    counts = null
  }
