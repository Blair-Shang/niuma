/**
 * Postgres 连接树脚本模板（对齐 Navicat / DBeaver / Vastbase PG 系常用集）。
 */
import { buildRoutineCallSql } from '@/modules/postgres/utils/routine-call'
...
/** 例程限定名：有 identity args 时带签名，便于重载区分。 */
export function qualifiedRoutineName(schema: string, name: string, args?: string): string {
  const qn = qualifiedName(schema, name)
  const sig = args?.trim()
  return sig ? `${qn}(${sig})` : qn
}

export function postgresSelectSeed(
...
export function postgresCountSeed(schema: string, table: string): string {
  return `SELECT COUNT(*) AS cnt\nFROM ${qualifiedName(schema, table)};\n`
}

export function postgresInsertSeed(
  schema: string,
  table: string,
...
export function postgresUpdateSeed(
  schema: string,
  table: string,
  columns: Array<{ name: string; dataType?: string }>,
  pkColumns: string[] = [],
...
export function postgresDeleteSeed(
  schema: string,
  table: string,
  pkColumns: string[] = [],
): string {
...
export function postgresTruncateSql(schema: string, table: string): string {
  return `TRUNCATE TABLE ${qualifiedName(schema, table)}`
}

export function postgresDropTableSql(schema: string, table: string, isView = false): string {
  const kw = isView ? 'VIEW' : 'TABLE'
  return `DROP ${kw} IF EXISTS ${qualifiedName(schema, table)}`
}

export function postgresVacuumSql(schema: string, table: string): string {
  return `VACUUM (VERBOSE, ANALYZE) ${qualifiedName(schema, table)};\n`
}

export function postgresAnalyzeSql(schema: string, table: string): string {
  return `ANALYZE VERBOSE ${qualifiedName(schema, table)};\n`
}

export function postgresRefreshMatViewSql(schema: string, table: string): string {
  return `REFRESH MATERIALIZED VIEW ${qualifiedName(schema, table)};\n`
}

export function postgresCallFunctionSeed(
  schema: string,
  name: string,
  args?: string,
): string {
...
export function postgresCallProcedureSeed(
  schema: string,
  name: string,
  args?: string,
): string {
...
export function postgresGrantSeed(
...
export function postgresDepsSql(schema: string, name: string): string {
...
export function postgresSequenceNextvalSeed(schema: string, name: string): string {
  return `SELECT nextval('${qualifiedName(schema, name)}'::regclass);\n`
}

export function postgresSequenceCurrvalSeed(schema: string, name: string): string {
  return `SELECT currval('${qualifiedName(schema, name)}'::regclass);\n`
}

export function postgresSequenceSetvalSeed(schema: string, name: string): string {
  return `SELECT setval('${qualifiedName(schema, name)}'::regclass, 1, false);\n`
}

export function postgresDropSequenceSql(schema: string, name: string): string {
  return `DROP SEQUENCE IF EXISTS ${qualifiedName(schema, name)};\n`
}

export function postgresBatchDropSql(
  items: Array<{
    schema: string
    name: string
...
export const selectSeed = postgresSelectSeed
