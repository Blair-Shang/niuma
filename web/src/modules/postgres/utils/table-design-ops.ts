...
import type {
  ...
  PostgresCreateTableCheck,
  PostgresCreateTableColumn,
  PostgresCreateTableForeignKey,
  PostgresCreateTableIndex,
  PostgresDesignOp,
  PostgresForeignKeyInfo,
  PostgresIndexInfo,
...
export function toDesignRows(cols: PostgresColumnInfo[], pkCols: string[]): DesignColumnDraft[] {
...
export function toIndexDrafts(indexes: PostgresIndexInfo[], pkCols: string[] = []): DesignIndexDraft[] {
...
export function toForeignKeyDrafts(fks: PostgresForeignKeyInfo[]): DesignForeignKeyDraft[] {
  return fks.map((fk) => {
...
function extractCheckExpression(c: PostgresConstraintInfo): string {
...
export function toCheckDrafts(constraints: PostgresConstraintInfo[]): DesignCheckDraft[] {
  return constraints
...
export function buildCreateColumns(rows: DesignColumnDraft[]): PostgresCreateTableColumn[] {
  return rows
    .filter((r) => !r.removed)
    .map((r) => {
      const dataType = syncColumnDataType(r)
      const col: PostgresCreateTableColumn = {
        name: r.name.trim(),
        dataType,
...
export function buildCreateIndexes(indexes: DesignIndexDraft[]): PostgresCreateTableIndex[] {
  return indexes
    .filter((r) => !r.removed && !r.primary && r.name.trim() && indexHasKeys(r))
    .map((r) => {
      const item: PostgresCreateTableIndex = {
        name: r.name.trim(),
        unique: r.unique,
      }
...
export function buildCreateForeignKeys(
  ...
  return fks
    .filter((r) => !r.removed)
    .map((r) => {
      const item: PostgresCreateTableForeignKey = {
...
export function buildCreateChecks(checks: DesignCheckDraft[]): PostgresCreateTableCheck[] {
  return checks
    .filter((r) => !r.removed && r.expression.trim())
    .map((r) => {
      const item: PostgresCreateTableCheck = { expression: r.expression.trim() }
      if (r.name.trim()) item.name = r.name.trim()
      return item
    })
}

function findPkConstraintName(constraints: PostgresConstraintInfo[]): string | undefined {
  return constraints.find((c) => c.type === 'p')?.name
}

function uniqueConstraintNames(constraints: PostgresConstraintInfo[]): Set<string> {
...
function pushAddIndex(ops: PostgresDesignOp[], idx: DesignIndexDraft): void {
  if (!idx.name.trim() || !indexHasKeys(idx)) return
  const op: PostgresDesignOp = {
    op: 'add_index',
    name: idx.name.trim(),
    unique: idx.unique,
  }
...
function pushAddForeignKey(ops: PostgresDesignOp[], fk: DesignForeignKeyDraft): void {
...
function pushAddCheck(ops: PostgresDesignOp[], ck: DesignCheckDraft): void {
