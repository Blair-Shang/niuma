/**
 * Postgres 例程调用 SQL：解析 identity args、所见即所得填参、生成 CALL/SELECT。
 * 对齐 Navicat；兼容 PostgreSQL 标准 `INOUT name type`，并容忍 `name INOUT type` 写法。
 */
import { ensureBatchExecMarker } from '@/modules/postgres/utils/query-exec-mode'
...
/** Oracle 风格：`name INOUT type`（mode 夹在名称与类型之间）。 */
...
export function serializeCallParams(params: RoutineCallParam[]): string {
  ...
  // 未填值也必须占位：空实参列表会变成 fn()，PostgreSQL 按 0 参重载解析，易误报 does not exist。
  return params
    .map((p) => {
 ...
 * 「运行调用」走 postgres.routine.call，不依赖本脚本。
