 ...
 * 表设计器草稿类型、索引方法与类型向导。
 */
export const POSTGRES_COMMON_DATA_TYPES: string[] = [
  'BIGINT',
  'BIGSERIAL',
  'INTEGER',
  'SERIAL',
...
export const POSTGRES_FK_ACTIONS = [
  'NO ACTION',
  'RESTRICT',
  'CASCADE',
  'SET NULL',
...
export type PostgresFkAction = (typeof POSTGRES_FK_ACTIONS)[number]

export const POSTGRES_INDEX_METHODS = [
  'btree',
  'hash',
  'gin',
  'gist',
...
export type PostgresIndexMethod = (typeof POSTGRES_INDEX_METHODS)[number]

export type DataTypeParamKind = 'none' | 'length' | 'precision'

export interface PostgresBaseTypeOption {
  base: string
...
export const POSTGRES_BASE_TYPE_OPTIONS: PostgresBaseTypeOption[] = [
  ...
  { base: 'TEXT', kind: 'none' },
  /** 未填长度时默认 VARCHAR(255)；需要无上限时请用 TEXT，或手动把长度改成其它值。 */
...
export function normalizeFkAction(raw: string): string {
  ...
  return (POSTGRES_FK_ACTIONS as readonly string[]).includes(a) ? a : 'NO ACTION'
}
...
function findBaseTypeOption(base: string): PostgresBaseTypeOption | undefined {
  const upper = base.trim().toUpperCase()
  return POSTGRES_BASE_TYPE_OPTIONS.find((o) => o.base.toUpperCase() === upper)
}

/** 解析基底类型对应的向导元数据（含 length / precision 约束）。 */
export function resolveBaseTypeOption(base: string): PostgresBaseTypeOption | undefined {
...
/** PostgreSQL IDENTITY 仅允许 smallint / integer / bigint（不含 SERIAL 伪类型）。 */
const IDENTITY_BASE_TYPES = new Set(['SMALLINT', 'INTEGER', 'BIGINT'])

/** 当前列类型是否允许 GENERATED ... AS IDENTITY。 */
export function supportsIdentityType(baseOrDataType: string): boolean {
  const raw = baseOrDataType.trim()
  if (!raw) return false
  const base = parseDataType(raw).base || raw
  return IDENTITY_BASE_TYPES.has(normalizeBaseType(base).toUpperCase())
}
...
export function composeDataType(input: {
  ...
  if (kind === 'length') {
    ...
    // 有默认长度（CHAR→1、VARCHAR→255）则补上；TIME / TIMESTAMP 允许无括号
