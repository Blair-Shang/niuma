/**
 * Postgres 调试辅助：日志点草稿（只生成文本，不执行 DDL）。
...
/** 已有调试日志点数量。 */
export function countPostgresDebugLogPoints(sql: string): number {
...
export function insertPostgresDebugLogPoint(
  ddl: string,
  options?: { label?: string },
): string {
  const src = ddl ?? ''
  const n = countPostgresDebugLogPoints(src) + 1
...
export function insertPostgresDebugLogPointParamGated(
  ddl: string,
  options?: { label?: string },
): string {
  const src = ddl ?? ''
 ...
 */
export function insertPostgresDebugLogPointAtLine(
  ddl: string,
  options: { line: number; label?: string },
): string {
  const src = ddl ?? ''
  if (!src.trim() || options.line < 1) {
    return insertPostgresDebugLogPoint(src, options)
  }
  const n = countPostgresDebugLogPoints(src) + 1
  const label = (options.label || `point_${n}`).trim() || `point_${n}`
  const block = buildLogBlock(label, sessionGateExpr())
...
export function wrapPostgresCallWithDebugSession(callSql: string): string {
  const lines = [
...
export function isPostgresDebugSessionScaffoldSql(sql: string): boolean {
...
export function renamePostgresRoutineInDdl(
  ddl: string,
