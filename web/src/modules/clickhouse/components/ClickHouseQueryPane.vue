...
const {
  ...
  onHistoryPick,
  onContextMenuSelect,
} = useClickHouseQueryPane(props)
</script>

<template>
  <SqlQueryShell
    v-model:history-open="historyOpen"
    class="nm-clickhouse-query"
    ...
    :show-explain="true"
    :show-explain-analyze="supportExplainAnalyze"
    @format="formatEditor"
  ...
  >
    ...
    <template #editor>
      <RsMonacoEditor
        ...
        height="100%"
        class="nm-clickhouse-query__editor"
        :options="{ automaticLayout: active !== false, minimap: { enabled: false } }"
      />
      <div v-else class="nm-clickhouse-query__editor-boot">
        <RsLoading size="sm" />
      </div>
    </template>
...
.nm-clickhouse-query__explain-mode {
  width: 118px;
  flex-shrink: 0;
  font-weight: 500;
}

.nm-clickhouse-query__editor {
  flex: 1;
  min-height: 0;
  border-radius: 0;
  border: none;
...
.nm-clickhouse-query__editor-boot {
  display: flex;
  flex: 1;
  align-items: center;
