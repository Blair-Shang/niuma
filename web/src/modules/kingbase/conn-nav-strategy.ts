...
function featureLabel(tab: KingbaseSessionTab): string {
  return i18n.global.t(kingbasePaneRegistry[tab].labelKey)
}

function resolveDesignMode(ctx?: ConnOpenContext): KingbaseObjectScriptMode {
  return ctx?.designMode === 'create' ? 'create' : 'alter'
}

function resolveObjectKind(ctx?: ConnOpenContext): KingbaseObjectKind | undefined {
...
function resolveObjectName(ctx?: ConnOpenContext, objectKind?: KingbaseObjectKind): string | undefined {
...
function objectScriptFeatureLabel(
  objectKind: KingbaseObjectKind | undefined,
  designMode: KingbaseObjectScriptMode,
): string {
  if (designMode === 'create') {
    if (objectKind === 'procedure') {
      return i18n.global.t('modules.kingbase.session.tabNewProcedure')
    }
    if (objectKind === 'function') {
      return i18n.global.t('modules.kingbase.session.tabNewFunction')
    }
    if (objectKind === 'sequence') {
      return i18n.global.t('modules.kingbase.session.tabNewSequence')
    }
    return i18n.global.t('modules.kingbase.session.tabNewView')
  }
  if (objectKind === 'procedure') {
    return i18n.global.t('modules.kingbase.session.tabProcedure')
  }
  if (objectKind === 'function') {
    return i18n.global.t('modules.kingbase.session.tabFunction')
  }
  if (objectKind === 'sequence') {
    return i18n.global.t('modules.kingbase.session.tabSequence')
  }
  return i18n.global.t('modules.kingbase.session.tabView')
}

function resolveFeature(ctx?: ConnOpenContext): KingbaseSessionTab {
  if (ctx?.initialTab) return normalizeKingbaseFeature(ctx.initialTab)
...
function buildCallTabSpec(item: ConnItem, ctx?: ConnOpenContext): ConnectionNavTabSpec {
  ...
  const title = routine
    ? `${routine} · ${label}`
    : [database, schema].filter(Boolean).join('.') || item.profileName
...
function buildQueryTabSpec(item: ConnItem, ctx?: ConnOpenContext): ConnectionNavTabSpec {
  ...
  const queryIndex = nextQueryTabIndex(useTabStore().allTabs, 'kingbase', item.profileId, () => true)
  const queryTitle = i18n.global.t('modules.kingbase.session.tabQueryIndexed', { n: queryIndex })
...
function buildDesignTabSpec(item: ConnItem, ctx?: ConnOpenContext): ConnectionNavTabSpec {
  ...
  return {
    ...
    icon: kingbasePaneRegistry.design.icon,
    props,
  }
}
...
function buildRelationTabSpec(
  ...
  return {
    ...
    title,
    tooltip: buildConnectionTabTooltip(item.profileName, item.hostAddress, resource, label),
    icon: kingbasePaneRegistry[feature].icon,
    props,
  }
}

function buildKingbaseTabSpec(item: ConnItem, ctx?: ConnOpenContext): ConnectionNavTabSpec {
...
export const kingbaseConnectionNavStrategy: ConnectionNavStrategy = {
  kind: 'kingbase',
  dedupFocus: true,
  buildTabSpec: buildKingbaseTabSpec,
  findExistingTab(tabs, _spec, item, ctx) {
    ...
    return tabs.find((tab: WorkspaceTab) => {
      if (tab.moduleId !== 'kingbase') return false
      if (tab.props.profileId !== item.profileId) return false
      if (normalizeKingbaseFeature(String(tab.props.initialTab ?? '')) !== feature) return false
