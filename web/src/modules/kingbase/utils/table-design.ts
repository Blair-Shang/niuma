 ...
 * 表设计器草稿类型、索引方法与类型向导。
 */
export const KINGBASE_COMMON_DATA_TYPES: string[] = [
  'BIGINT',
  'BIGSERIAL',
  'INTEGER',
  'SERIAL',
...
export const KINGBASE_FK_ACTIONS = [
  'NO ACTION',
  'RESTRICT',
  'CASCADE',
  'SET NULL',
...
export type KingbaseFkAction = (typeof KINGBASE_FK_ACTIONS)[number]

export const KINGBASE_INDEX_METHODS = [
  'btree',
  'hash',
  'gin',
  'gist',
...
export type KingbaseIndexMethod = (typeof KINGBASE_INDEX_METHODS)[number]

export type DataTypeParamKind = 'none' | 'length' | 'precision'

export interface KingbaseBaseTypeOption {
  base: string
...
export const KINGBASE_BASE_TYPE_OPTIONS: KingbaseBaseTypeOption[] = [
...
export function normalizeFkAction(raw: string): string {
  ...
  return (KINGBASE_FK_ACTIONS as readonly string[]).includes(a) ? a : 'NO ACTION'
}
...
function findBaseTypeOption(base: string): KingbaseBaseTypeOption | undefined {
  const upper = base.trim().toUpperCase()
  return KINGBASE_BASE_TYPE_OPTIONS.find((o) => o.base.toUpperCase() === upper)
}

/** 解析基底类型对应的向导元数据（含 length / precision 约束）。 */
export function resolveBaseTypeOption(base: string): KingbaseBaseTypeOption | undefined {
