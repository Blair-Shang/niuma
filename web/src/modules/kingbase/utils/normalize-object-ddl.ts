/**
 * 从 Kingbase CREATE VIEW / FUNCTION / PROCEDURE / SEQUENCE 正文解析对象名。
 * 支持 schema.name 与 "schema"."name"；有 schema 时取 name 段。
 */
import type { KingbaseObjectKind } from '@/modules/kingbase/types/object-script'
...
export function parseKingbaseObjectNameFromSql(
  sql: string,
  kind: KingbaseObjectKind,
): string | null {
  let s = (sql ?? '').trim()
  if (!s) return null
