/**
 * Kingbase 查询编辑器：Bridge LSP（kingbaseparser）+ 静默回退。
 *
...
import {
  attachKingbaseSqlLsp,
  bootstrapKingbaseMonaco,
} from '@/modules/kingbase/monaco-bootstrap'

/** 多面板共用：bootstrapKingbaseMonaco 内部已幂等 */
let sharedBootstrapped = false
let editorSeq = 0

export function useKingbaseSqlEditor(options: {
  sqlText: Ref<string>
  active: () => boolean
  onRun: () => void
  ...
  const editorId = `kingbase-editor-${++editorSeq}`
  ...
  async function syncLspAttachment(): Promise<void> {
    ...
    try {
      detach = await attachKingbaseSqlLsp({
        model,
        sessionId,
        editorId,
        database,
