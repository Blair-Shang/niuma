...
import type { KingbaseDatabaseCreateOptionsResult, KingbaseDdlParams } from '@/api/types/kingbase'
import {
  useKingbaseDdlActionStore,
  type KingbaseDatabaseCreateOptions,
  type KingbasePendingDdlAction,
} from '@/modules/kingbase/stores/ddl-actions'
...
/** 新建数据库表单状态与候选项加载。 */
export function useKingbaseDatabaseCreateForm() {
  const { t } = useI18n()
  const toast = useRsToast()
  const { pending, busy } = storeToRefs(useKingbaseDdlActionStore())
  ...
  const defaultOptionLabel = computed(() => t('modules.kingbase.ddl.dbDefaultOption'))
  ...
  function applyCreateOptions(result: KingbaseDatabaseCreateOptionsResult): void {
  ...
  function applyCreateDefaults(
    ...
    opts: KingbaseDatabaseCreateOptions | undefined,
  ): void {
