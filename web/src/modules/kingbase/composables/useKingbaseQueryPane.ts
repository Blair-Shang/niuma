/**
 * Kingbase 查询面板：走 Kingbase.query.* RPC。
...
import { resolveQueryExecMode, type KingbaseQueryExecMode } from '@/modules/kingbase/utils/query-exec-mode'
import { useSessionRegistry } from '@/stores/session-registry'

export type KingbaseQueryPaneProps = {
...
export function useKingbaseQueryPane(props: KingbaseQueryPaneProps) {
  const { t } = useI18n()
  ...
  const gridTabs = shallowRef<KingbaseGridTab[]>([])
  ...
  function dialectProfile() {
    ...
    return sessions.getDialectForSession(props.sessionId) ?? defaultKingbaseProfile()
  }

  const editor = useKingbaseSqlEditor({
    sqlText,
  ...
  const { historyOpen, historyEntries, rememberSql, onHistoryPick } = useSqlQueryHistory({
    ...
    storagePrefix: 'niuma.kingbase.sqlHistory.',
    sqlText,
  })

  const identityTitle = computed(() =>
    [
      props.sessionLabel || 'Kingbase',
      props.database?.trim(),
      props.schema?.trim(),
    ]
      .filter(Boolean)
  ...
  const activeGrid = computed(
    (): KingbaseGridTab | null => gridTabs.value.find((g) => g.id === activePaneTab.value) ?? null,
  )

  const resultColumns = computed((): RsTableColumn[] =>
    (activeGrid.value?.columns ?? []).map((c, i) => {
      const name = c.name || `col${i + 1}`
      const tipLines = [t('modules.kingbase.query.colTipField', { name })]
      if (c.dataType?.trim()) {
        tipLines.push(t('modules.kingbase.query.colTipType', { type: c.dataType.trim() }))
      }
  ...
  const resultSummaryText = computed(() => {
    ...
    if (grid) {
      return t('modules.kingbase.query.resultSummary', {
        rows: grid.rowCount,
        cols: grid.columns.length,
        ms: grid.durationMs,
      })
    }
    if (lastError.value) return t('modules.kingbase.query.execFailed')
  ...
  const messageItems = computed((): QueryResultMessageItem[] => {
    ...
    if (lastError.value) {
      items.push({ key: 'error', label: t('modules.kingbase.query.msgError'), value: lastError.value, tone: 'error' })
      return items
    }
    if (lastExecSummary.value) {
      items.push({ key: 'summary', label: t('modules.kingbase.query.msgOk'), value: lastExecSummary.value, tone: 'success' })
    }
    lastNotices.value.forEach((notice, index) => {
      items.push({
        key: `notice-${index}`,
        label: t('modules.kingbase.query.msgNotice'),
        value: notice,
        tone: 'default',
      })
    })
    ...
    if (grid) {
      items.push({
        key: 'grid',
        label: t('modules.kingbase.query.resultTab'),
        value: t('modules.kingbase.query.resultSummary', { rows: grid.rowCount, cols: grid.columns.length, ms: grid.durationMs }),
        tone: 'default',
      })
      if (grid.hasMore) {
        items.push({ key: 'has-more', label: t('modules.kingbase.query.hasMore'), value: '', tone: 'warning' })
      }
      if (grid.truncated) {
        items.push({
          key: 'truncated',
          label: t('modules.kingbase.query.truncated'),
          value: t('modules.kingbase.query.truncatedCap', { count: grid.fetchedCount }),
          tone: 'warning',
        })
      }
    }
  ...
  async function refreshTxState(): Promise<void> {
    ...
    try {
      const state = await kingbaseApi.txGetState({ sessionId: props.sessionId })
  ...
  async function setAutoCommit(enabled: boolean): Promise<void> {
    ...
    txBusy.value = true
    try {
      const state = await kingbaseApi.txSetAutoCommit({
        sessionId: props.sessionId,
        autoCommit: enabled,
      })
      autoCommit.value = state.autoCommit
      inTransaction.value = state.inTransaction
      if (enabled) {
        toast.info(t('modules.kingbase.query.autoCommitOnHint'))
      } else {
        toast.info(t('modules.kingbase.query.autoCommitOffHint'))
      }
    } catch (e) {
  ...
  async function commitTx(): Promise<void> {
    ...
    txBusy.value = true
    try {
      const state = await kingbaseApi.txCommit({ sessionId: props.sessionId })
      autoCommit.value = state.autoCommit
      inTransaction.value = state.inTransaction
      toast.success(t('modules.kingbase.query.commitDone'))
    } catch (e) {
      toast.error(e instanceof Error ? e.message : String(e))
      await refreshTxState()
    } finally {
  ...
  async function rollbackTx(): Promise<void> {
    ...
    txBusy.value = true
    try {
      const state = await kingbaseApi.txRollback({ sessionId: props.sessionId })
      autoCommit.value = state.autoCommit
      inTransaction.value = state.inTransaction
      toast.success(t('modules.kingbase.query.rollbackDone'))
    } catch (e) {
      toast.error(e instanceof Error ? e.message : String(e))
      await refreshTxState()
    } finally {
  ...
  async function closeResultSetQuiet(rsId: string | null | undefined): Promise<void> {
    ...
    try {
      await kingbaseApi.queryClose({ sessionId: props.sessionId, resultSetId: rsId })
    } catch {
      // 静默
    }
  }
  ...
  function addGridTab(result: KingbaseQueryExecResult, stmtIndex: number, stmtSql: string): KingbaseGridTab {
    ...
    const tab: KingbaseGridTab = {
      ...
      label: t('modules.kingbase.query.resultTabIndexed', { n: stmtIndex + 1 }),
      stmtIndex,
    }
    ...
    if (tabs.length >= MAX_RESULT_GRID_TABS) {
      ...
      toast.warning(
        t('modules.kingbase.query.batchTabsTrimmed', { n: trimCount, max: MAX_RESULT_GRID_TABS }),
      )
  ...
  function ingestResult(
    result: KingbaseQueryExecResult,
    stmtIndex: number,
    stmtSql: string,
  ): void {
    ...
    lastExecSummary.value =
      n > 0
        ? t('modules.kingbase.query.affected', { n })
        : t('modules.kingbase.query.rows', { n: 0 })
  ...
  async function runBatchStatements(
    ...
    try {
      ...
      if (/unknown method|not found|execBatch/i.test(msg)) {
        throw new Error(
          '需要重启 NiuMa 以加载 kingbase.query.execBatch（含 -- niuma:exec=batch 的脚本须同连接执行）',
        )
      }
      throw batchErr
    }
  ...
  async function runSql(): Promise<void> {
    ...
    if (!sql.trim()) {
      toast.warning(t('modules.kingbase.query.empty'))
      return
    }
    running.value = true
    ...
    const requestId = `Kingbase-${Date.now()}`
  ...
  async function runExplain(analyze: boolean): Promise<void> {
    ...
    if (!props.sessionId) {
      lastError.value = t('modules.kingbase.query.noSession')
      activePaneTab.value = 'messages'
      return
    }
    const sql = editor.resolveSql()
    ...
    try {
      ...
      const result = await kingbaseApi.queryExplain({
        sessionId: props.sessionId,
        database: props.database?.trim() || undefined,
        sql: target,
        analyze,
      ...
      lastExecSummary.value = [
        analyze
          ? t('modules.kingbase.query.explainAnalyzeDone')
          : t('modules.kingbase.query.explainDone'),
        t('modules.kingbase.query.resultSummary', {
  ...
  async function fetchMore(): Promise<void> {
    ...
    try {
      ...
      toast.error(t('modules.kingbase.query.fetchError'))
    } finally {
  ...
  function exportCsv(): void {
    const grid = activeGrid.value
    if (!grid) {
      toast.warning(t('modules.kingbase.query.noResult'))
      return
    }
    ...
    a.href = url
    a.download = 'Kingbase-export.csv'
    a.click()
  ...
  const contextMenuItems = computed((): RsContextMenuItem[] =>
    buildSqlQueryContextMenuItems({
      labels: {
        run: t('modules.kingbase.query.run'),
        runSelection: t('modules.kingbase.query.runSelection'),
        cancel: t('modules.kingbase.query.cancel'),
        format: t('modules.kingbase.query.format'),
        compress: t('modules.kingbase.query.format'),
        copy: t('modules.kingbase.query.run'),
        paste: t('modules.kingbase.query.run'),
        explain: t('modules.kingbase.query.explain'),
        explainAnalyze: t('modules.kingbase.query.explainAnalyze'),
        exportCsv: t('modules.kingbase.query.exportCsv'),
        fetchMore: t('modules.kingbase.query.loadMore'),
        fetchAll: t('modules.kingbase.query.fetchAll'),
      },
  ...
  const toolbarLabels = computed((): SqlQueryToolbarLabels => ({
    toolbarAria: 'Kingbase',
    run: t('modules.kingbase.query.run'),
    runSelection: t('modules.kingbase.query.runSelection'),
    runTooltip: t('modules.kingbase.query.runHint'),
    cancel: t('modules.kingbase.query.cancel'),
    cancelTooltip: t('modules.kingbase.query.cancel'),
    format: t('modules.kingbase.query.format'),
    formatTooltip: t('modules.kingbase.query.formatTooltip'),
    explain: t('modules.kingbase.query.explain'),
    explainTooltip: t('modules.kingbase.query.explainHint'),
    explainAnalyze: t('modules.kingbase.query.explainAnalyze'),
    explainAnalyzeTooltip: t('modules.kingbase.query.explainAnalyzeHint'),
    history: t('modules.kingbase.query.history'),
    historyEmpty: t('modules.kingbase.query.historyEmpty'),
    historyClear: t('modules.kingbase.query.historyClear'),
    autoCommit: t('modules.kingbase.query.autoCommit'),
    autoCommitTooltip: t('modules.kingbase.query.autoCommitTooltip'),
    commit: t('modules.kingbase.query.commit'),
    commitTooltip: t('modules.kingbase.query.commitTooltip'),
    rollback: t('modules.kingbase.query.rollback'),
    rollbackTooltip: t('modules.kingbase.query.rollbackTooltip'),
    inTransaction: t('modules.kingbase.query.inTransaction'),
  }))

  const resultPanelLabels = computed((): QueryResultPanelLabels => ({
    messages: t('modules.kingbase.query.messages'),
    messagesEmpty: t('modules.kingbase.query.messagesEmpty'),
    filterPlaceholder: t('modules.kingbase.query.filterPlaceholder'),
    loadMore: t('modules.kingbase.query.loadMore'),
    fetchAll: t('modules.kingbase.query.fetchAll'),
    exportCsv: t('modules.kingbase.query.exportCsv'),
    emptyResult: t('modules.kingbase.query.emptyResult'),
    resultEmpty: t('modules.kingbase.query.resultEmpty'),
    closeResultTab: t('modules.kingbase.query.closeResultTab'),
    batchResultTab: (n: number) => t('modules.kingbase.query.resultTabIndexed', { n }),
    tabRowCount: (n: number, more: boolean) =>
      more ? t('modules.kingbase.query.rows', { n }) + '+' : t('modules.kingbase.query.rows', { n }),
    batchStmtLabel: (n: number) => t('modules.kingbase.query.resultTabIndexed', { n }),
    batchStmtSkipped: '',
    batchStmtRunning: '',
    batchStmtPending: '',
    batchOpenResult: '',
    ...
    logColTime: '',
    logColRows: '',
    msgOk: t('modules.kingbase.query.msgOk'),
    msgError: t('modules.kingbase.query.msgError'),
    cancelled: t('modules.kingbase.query.cancelled'),
  }))
