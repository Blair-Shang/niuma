...
import type { KingbaseIoDoneEvent, KingbaseIoProgressEvent } from '@/api/types/kingbase'

export interface KingbaseIoTaskLine {
  taskId: string
  phase: string
  message: string
  ok?: boolean
 ...
 */
export function useKingbaseIoTasks(prefix = 'kingbase.io.') {
  const activeTaskId = ref<string | null>(null)
  const lastMessage = ref('')
  const lastOutputPath = ref<string | undefined>()
  const lines = ref<KingbaseIoTaskLine[]>([])
  let offEvent: (() => void) | null = null
  let trackedTaskId: string | null = null
  /** track() 之后、waitForTask 之前短暂为 true */
  let acceptingEarly = false
  let resolveDone: ((ev: KingbaseIoDoneEvent) => void) | null = null
  const earlyDones = new Map<string, KingbaseIoDoneEvent>()

  function pushLine(line: Omit<KingbaseIoTaskLine, 'at'>): void {
  ...
  function applyDone(done: KingbaseIoDoneEvent): void {
  ...
  function ensureSubscribed(): void {
    ...
    offEvent = subscribeBridgeEventByPrefix(prefix, (detail) => {
      ...
      const event = detail as KingbaseIoProgressEvent | KingbaseIoDoneEvent
      if (event.type.endsWith('.progress')) {
        const progress = event as KingbaseIoProgressEvent
      ...
      const done = event as KingbaseIoDoneEvent
      if (trackedTaskId) {
  ...
  function waitForTask(taskId: string): Promise<KingbaseIoDoneEvent> {
    ensureSubscribed()
