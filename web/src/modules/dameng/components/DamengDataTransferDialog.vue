...
} from '@/modules/dameng/utils/csv-header'
...
const windowTitle = computed(() => task.value?.title ?? t('modules.dameng.io.exportTitle'))
...
const shellLabels = computed(
  (): DataTransferShellLabels => ({
    ...
    confirm: isExport.value ? t('modules.dameng.io.export') : t('modules.dameng.io.import'),
  }),
)
...
const delimiterOptions = computed<RsSelectOptions>(() => [
  { value: ',', label: t('modules.dameng.io.delimiterComma') },
  { value: '\t', label: t('modules.dameng.io.delimiterTab') },
  { value: ';', label: t('modules.dameng.io.delimiterSemicolon') },
  { value: '|', label: t('modules.dameng.io.delimiterPipe') },
])

const targetOptions = computed<RsSelectOptions>(() => [
  { value: '', label: t('modules.dameng.io.columnMapSkip') },
...
async function loadTableColumns(): Promise<void> {
  ...
  try {
    const result = await damengApi.metaColumns({
...
async function loadColumnMapFromFile(): Promise<void> {
  ...
  try {
    ...
    if (sources.length === 0) {
      resetMap()
      toast.error(t('modules.dameng.io.columnMapEmpty'))
      return
    }
  ...
  try {
    if (current.kind === 'export_csv') {
      const result = await dialogApi.saveFile({
        title: t('modules.dameng.io.browseExportTitle'),
      ...
      const result = await dialogApi.openFile({
        title: t('modules.dameng.io.browseImportTitle'),
        accept: ['.csv'],
      })
...
async function onConfirm(): Promise<void> {
  ...
  try {
    ...
    toast.success(isExport.value ? t('modules.dameng.io.exportDone') : t('modules.dameng.io.importDone'))
    if (isExport.value) {
  ...
  >
    <DataTransferPanel :labels="panelLabels" :lines="lines" :busy="busy">
      <DataTransferSection :title="t('modules.dameng.io.sectionTarget')">
        <div class="nm-dameng-dt__scope" :title="scopeLabel">{{ scopeLabel }}</div>
      </DataTransferSection>
      ...
      <DataTransferSection :title="t('modules.dameng.io.sectionFormat')">
        <div class="nm-dameng-dt__grid">
          <div class="nm-dameng-dt__field">
            <RsLabel>{{ t('modules.dameng.io.delimiter') }}</RsLabel>
            <RsSelect v-model="delimiter" :options="delimiterOptions" :disabled="busy" />
          </div>
          <div class="nm-dameng-dt__field">
            <RsLabel>{{ t('modules.dameng.io.nullString') }}</RsLabel>
            <RsInput
              v-model="nullString"
              :disabled="busy"
              :placeholder="t('modules.dameng.io.nullStringPh')"
            />
          </div>
        </div>
        <div class="nm-dameng-dt__checks">
          <DataTransferCheck
            v-model="header"
            variant="chip"
            :label="t('modules.dameng.io.header')"
            :disabled="busy"
          />
          <DataTransferCheck
            v-if="!isExport"
            v-model="truncate"
            variant="chip"
            :label="t('modules.dameng.io.truncateFirst')"
            :disabled="busy"
          />
        </div>
      </DataTransferSection>
      ...
      <DataTransferSection
        v-if="!isExport"
        :title="t('modules.dameng.io.columnMapTitle')"
      >
        <template #head>
          <RsButton
            size="sm"
          ...
          >
            {{ t('modules.dameng.io.columnMapLoad') }}
          </RsButton>
        </template>
        <p class="nm-dameng-dt__map-hint">{{ t('modules.dameng.io.columnMapHint') }}</p>
        <p v-if="sourceColumns.length" class="nm-dameng-dt__map-meta">
          {{ t('modules.dameng.io.columnMapMapped', { n: mappedCount, total: sourceColumns.length }) }}
        </p>
        <div v-if="sourceColumns.length" class="nm-dameng-dt__map">
          <div class="nm-dameng-dt__map-head">
            <span>{{ t('modules.dameng.io.columnMapSource') }}</span>
            <span>{{ t('modules.dameng.io.columnMapTarget') }}</span>
          </div>
          <div
            v-for="src in sourceColumns"
            :key="src"
            class="nm-dameng-dt__map-row"
          >
            <span class="nm-dameng-dt__map-src" :title="src">{{ src }}</span>
            <RsSelect
        ...
        <p v-else class="nm-dameng-dt__map-empty">{{ t('modules.dameng.io.columnMapNeedFile') }}</p>
      </DataTransferSection>

      <template #note>
        {{
          isExport ? t('modules.dameng.io.csvExportHint') : t('modules.dameng.io.csvImportHint')
        }}
      </template>
    </DataTransferPanel>
  </DataTransferShell>
...
.nm-dameng-dt__map-hint,
.nm-dameng-dt__map-meta,
.nm-dameng-dt__map-empty {
  margin: 0;
...
.nm-dameng-dt__map-head,
.nm-dameng-dt__map-row {
  display: grid;
