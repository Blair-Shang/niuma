...
/**
 * 达梦表设计器：挂载公共 TableDesignShell，方言负责草稿 / RPC / 类型目录。
...
import {
  DAMENG_BASE_TYPE_OPTIONS,
  DAMENG_FK_ACTIONS,
  DAMENG_INDEX_METHODS,
  dataTypeParamKind,
...
} from '@/modules/dameng/utils/table-design'
...
import {
  ...
  toForeignKeyDrafts,
  toIndexDrafts,
} from '@/modules/dameng/utils/table-design-ops'
...
const shellLabels = computed<TableDesignShellLabels>(() => ({
  reload: t('modules.dameng.design.reload'),
  preview: t('modules.dameng.design.preview'),
  create: t('modules.dameng.design.create'),
  apply: t('modules.dameng.design.apply'),
  previewTitle: t('modules.dameng.design.previewTitle'),
  selectRow: t('modules.dameng.design.selectRow'),
  copyPreview: t('modules.dameng.design.copyPreview'),
  moveUp: t('modules.dameng.design.moveUp'),
  moveDown: t('modules.dameng.design.moveDown'),
  add: t('modules.dameng.design.addColumn'),
}))

const addButtonLabel = computed(() => {
  if (activeSection.value === 'indexes') return t('modules.dameng.design.addIndex')
  if (activeSection.value === 'foreignKeys') return t('modules.dameng.design.addForeignKey')
  if (activeSection.value === 'checks') return t('modules.dameng.design.addCheck')
  return t('modules.dameng.design.addColumn')
})

const title = computed(() =>
  modeCreate.value
    ? t('modules.dameng.design.createTitle')
    : t('modules.dameng.design.alterTitle', { name: effectiveTable.value ?? '' }),
)

const sections = computed<TableDesignSectionItem[]>(() => [
  {
    id: 'columns',
    label: t('modules.dameng.design.tabColumns'),
    count: columns.value.filter((c) => !c.removed).length,
  },
  {
    id: 'indexes',
    label: t('modules.dameng.design.tabIndexes'),
    count: indexes.value.filter((i) => !i.removed).length,
  },
  {
    id: 'foreignKeys',
    label: t('modules.dameng.design.tabForeignKeys'),
    count: foreignKeys.value.filter((f) => !f.removed).length,
  },
  {
    id: 'checks',
    label: t('modules.dameng.design.tabChecks'),
    count: checks.value.filter((c) => !c.removed).length,
  },
])

const indexMethodOptions: RsSelectOptions = DAMENG_INDEX_METHODS.map((v) => ({
  value: v,
  label: v,
}))

const typeBaseSelectOptions = DAMENG_BASE_TYPE_OPTIONS.map((o) => ({
  value: o.base,
  label: o.base,
}))

const fkActionOptions = DAMENG_FK_ACTIONS.map((v) => ({ value: v, label: v }))
...
const displayColumns = computed((): ColRow[] =>
  columns.value
    .filter((c) => !c.removed)
    .map((c) => {
      let status = t('modules.dameng.design.statusOk')
      if (!c.originalName) status = t('modules.dameng.design.statusNew')
      else {
        ...
        if (
          ...
          status = t('modules.dameng.design.statusEdit')
        }
      }
      return { ...c, status }
    }) as ColRow[],
  ...
  (): IdxRow[] =>
    indexes.value
      ...
      .map((i) => ({
        ...i,
        kindLabel: i.primary
          ? t('modules.dameng.design.idxKindPrimary')
          : i.unique
            ? t('modules.dameng.design.idxKindUnique')
            : t('modules.dameng.design.idxKindNormal'),
      })) as IdxRow[],
)
...
const columnColumns = computed((): RsTableColumn<ColRow>[] => {
  ...
  { key: 'name', title: t('modules.dameng.design.colName'), minWidth: 110, editable: true },
  {
    key: 'typeBase',
    title: t('modules.dameng.design.colType'),
    minWidth: 120,
    editable: true,
    valueType: 'select',
  ...
  {
    key: 'typeLength',
    title: t('modules.dameng.design.colLength'),
    width: 72,
    align: 'center',
    valueType: 'number',
    editable: (row) => {
  ...
  {
    key: 'typeScale',
    title: t('modules.dameng.design.colScale'),
    width: 64,
    align: 'center',
    valueType: 'number',
  ...
  {
    key: 'primaryKey',
    title: t('modules.dameng.design.colPk'),
    width: 52,
    align: 'center',
    editable: true,
    valueType: 'boolean',
  ...
  {
    key: 'nullable',
    title: t('modules.dameng.design.colNullable'),
    width: 52,
    align: 'center',
    editable: true,
    valueType: 'boolean',
  ...
  {
    key: 'autoIncrement',
    title: t('modules.dameng.design.colAi'),
    width: 52,
    align: 'center',
    editable: true,
    valueType: 'boolean',
  ...
  {
    key: 'defaultExpr',
    title: t('modules.dameng.design.colDefault'),
    minWidth: 100,
    editable: true,
    ellipsis: true,
  },
  {
    key: 'comment',
    title: t('modules.dameng.design.colComment'),
    minWidth: 100,
    editable: true,
    ellipsis: true,
  },
  ...
  if (!modeCreate.value) {
    cols.push({
      key: 'status',
      title: t('modules.dameng.design.colStatus'),
      width: 72,
      align: 'center',
    })
  }
...
const indexColumns = computed((): RsTableColumn<IdxRow>[] => [
  { key: 'name', title: t('modules.dameng.design.idxName'), minWidth: 110, editable: true },
  {
    key: 'kindLabel',
    title: t('modules.dameng.design.idxKind'),
    width: 88,
    editable: false,
  },
  {
    key: 'columnsText',
    title: t('modules.dameng.design.idxColumns'),
    minWidth: 160,
    editable: true,
    valueType: 'select',
    headerTip: t('modules.dameng.design.idxColumnsTip'),
    editorOptions: {
  ...
  {
    key: 'method',
    title: t('modules.dameng.design.idxMethod'),
    width: 88,
    editable: true,
    valueType: 'select',
  ...
  {
    key: 'unique',
    title: t('modules.dameng.design.idxUnique'),
    width: 64,
    align: 'center',
    editable: true,
    valueType: 'boolean',
...
const fkColumns = computed((): RsTableColumn<FkRow>[] => [
  { key: 'name', title: t('modules.dameng.design.fkName'), minWidth: 100, editable: true },
  {
    key: 'columnsText',
    title: t('modules.dameng.design.fkColumns'),
    minWidth: 140,
    editable: true,
    valueType: 'select',
    editorOptions: {
  ...
  {
    key: 'refSchema',
    title: t('modules.dameng.design.fkRefSchema'),
    minWidth: 110,
    editable: true,
    valueType: 'select',
    editorOptions: {
  ...
  {
    key: 'refTable',
    title: t('modules.dameng.design.fkRefTable'),
    minWidth: 120,
    editable: true,
    valueType: 'select',
    editorOptions: {
  ...
  {
    key: 'refColumnsText',
    title: t('modules.dameng.design.fkRefColumns'),
    minWidth: 140,
    editable: true,
    valueType: 'select',
    editorOptions: {
  ...
  {
    key: 'onDelete',
    title: t('modules.dameng.design.fkOnDelete'),
    width: 110,
    editable: true,
    valueType: 'select',
  ...
  {
    key: 'onUpdate',
    title: t('modules.dameng.design.fkOnUpdate'),
    width: 110,
    editable: true,
    valueType: 'select',
    editorOptions: { options: fkActionOptions },
...
const checkColumns = computed((): RsTableColumn<ChkRow>[] => [
  {
    key: 'name',
    title: t('modules.dameng.design.chkName'),
    minWidth: 120,
    editable: true,
  },
  {
    key: 'expression',
    title: t('modules.dameng.design.chkExpression'),
    minWidth: 220,
    editable: true,
    ellipsis: true,
  },
...
function onColCommit(row: ColRow, column: RsTableColumn<ColRow>, _i: number, value: unknown): void {
  ...
  patchColumn(row.__rowKey, (r) => {
    ...
    if (key === 'typeBase') {
      ...
      const opt = DAMENG_BASE_TYPE_OPTIONS.find((o) => o.base === nextBase)
...
function onIdxCommit(row: IdxRow, column: RsTableColumn<IdxRow>, _i: number, value: unknown): void {
  ...
  indexes.value = indexes.value.map((r) => {
    ...
    if (key === 'method') {
      return { ...r, method: normalizeIndexMethod(String(value ?? 'BTREE')) }
    }
...
async function ensureRefTables(schema?: string): Promise<void> {
  if (!props.profileId && !props.sessionId) return
  const sch = (schema ?? props.schema).trim()
  if (!sch) {
...
async function ensureRefColumns(table: string, schema?: string): Promise<void> {
  ...
  try {
    const base = props.sessionId
      ? { sessionId: props.sessionId, schema: sch, table }
      : { profileId: props.profileId!, schema: sch, table }
    const result = await damengApi.metaColumns(base)
...
function onFkCommit(row: FkRow, column: RsTableColumn<FkRow>, _i: number, value: unknown): void {
  ...
  foreignKeys.value = foreignKeys.value.map((r) => {
    ...
    if (key === 'refSchema') {
      const schema = draft || props.schema
    ...
    if (key === 'onDelete') {
      ...
      return {
        ...r,
        onDelete: (DAMENG_FK_ACTIONS as readonly string[]).includes(upper) ? upper : r.onDelete,
      }
    }
    if (key === 'onUpdate') {
      const upper = draft.toUpperCase()
      return {
        ...r,
        onUpdate: (DAMENG_FK_ACTIONS as readonly string[]).includes(upper) ? upper : r.onUpdate,
      }
    }
    return r
  })
...
async function copyPreviewSql(): Promise<void> {
  ...
  try {
    ...
    toast.success(t('modules.dameng.design.copyPreviewOk'))
  } catch {
    toast.error(t('modules.dameng.design.copyPreviewFailed'))
  }
}
...
async function load(): Promise<void> {
  ...
  try {
    ...
    const [colsResult, idxsResult, pkResult, fkResult, chkResult] = await Promise.all([
      damengApi.metaColumns(base),
      damengApi.metaIndexes(base),
      damengApi.metaPrimaryKey(base),
      damengApi.metaForeignKeys(base).catch(() => ({ foreignKeys: [] })),
      damengApi.metaChecks(base).catch(() => ({ checks: [] })),
    ])
  ...
  try {
    if (modeCreate.value) {
      if (!tableName.value.trim()) {
        toast.error(t('modules.dameng.design.needTableName'))
        return false
      }
      const cols = buildCreateColumns(columns.value)
      if (cols.length === 0) {
        toast.info(t('modules.dameng.design.needColumns'))
        return false
      }
      const result = await damengApi.ddlCreateTablePreview(buildCreatePayload())
      previewSqls.value = formatPreviewSqls(result.sql ?? [])
    } else {
      const ops = currentAlterOps()
      if (ops.length === 0) {
        // 无变更也打开预览，用 SQL 注释说明，避免 Popover 一闪即关
        previewSqls.value = [`-- ${t('modules.dameng.design.noChanges')}`]
        return true
      }
      const result = await damengApi.ddlDesignPreview({
...
function formatPreviewSqls(sqls: string[]): string[] {
  return sqls.map((s) => {
    ...
    if (!raw) return raw
    try {
      return formatSql(raw, { dialect: 'dameng' })
    } catch {
      return raw
    }
  })
...
function switchToAlterAfterCreate(name: string): void {
  ...
  if (tabId) {
    ...
    const designLabel = t('modules.dameng.session.tabDesign')
...
async function onApply(): Promise<void> {
  ...
  try {
    if (modeCreate.value) {
      ...
      await damengApi.ddlCreateTable(buildCreatePayload())
      toast.success(t('modules.dameng.design.createOk', { name }))
      showPreview.value = false
      ...
      const ops = currentAlterOps()
      if (ops.length === 0) {
        toast.info(t('modules.dameng.design.noChanges'))
        return
      }
      const table = effectiveTable.value
      if (!table) {
      ...
      await damengApi.ddlDesignApply({
        sessionId: props.sessionId,
        schema: props.schema,
        name: table,
        ops,
      })
      toast.success(t('modules.dameng.design.applyOk'))
      showPreview.value = false
      await load()
    }
  } catch (e) {
...
<template>
  ...
  >
    ...
    <template #meta>
      <div class="nm-dameng-design__meta-row">
        <label class="nm-dameng-design__meta-label">{{ t('modules.dameng.design.tableName') }}</label>
        <RsInput
          v-if="modeCreate"
          v-model="tableName"
          size="sm"
          :placeholder="t('modules.dameng.design.tableNamePh')"
        />
        <span v-else class="nm-dameng-design__meta-readonly">{{ table || tableName }}</span>
      </div>
      <div class="nm-dameng-design__meta-row nm-dameng-design__meta-row--full">
        <label class="nm-dameng-design__meta-label">{{ t('modules.dameng.design.tableComment') }}</label>
        <RsInput
          v-model="tableComment"
          size="sm"
          :placeholder="t('modules.dameng.design.tableCommentPh')"
        />
      </div>
    </template>
            ...
            (row) =>
              row
                ? [
                    {
                      key: 'remove',
                      label: t('modules.dameng.design.remove'),
                      icon: 'trash-2',
                      danger: true,
                    },
                  ]
                ...
                ? [
                    {
                      ...
                      label: row.originalName
                        ? t('modules.dameng.design.dropCheck')
                        : t('modules.dameng.design.removeCheck'),
                      icon: 'trash-2',
                      danger: true,
                    },
                  ]
    ...
    <template #editor>
      <template v-if="activeSection === 'columns' && editingCol">
        <div class="nm-dameng-design__editor-title">{{ t('modules.dameng.design.editColumn') }}</div>
        <div class="nm-dameng-design__form">
          <div class="nm-dameng-design__field">
            <label>{{ t('modules.dameng.design.colName') }}</label>
            <RsInput
          ...
          <div class="nm-dameng-design__field">
            <label>{{ t('modules.dameng.design.colType') }}</label>
            <RsSelect
          ...
          <div class="nm-dameng-design__field">
            <label>{{ t('modules.dameng.design.colDefault') }}</label>
            <RsInput
              :model-value="editingCol.defaultExpr"
              size="sm"
              :placeholder="t('modules.dameng.design.colDefaultPh')"
              @update:model-value="
          ...
          <div class="nm-dameng-design__field">
            <label>{{ t('modules.dameng.design.colComment') }}</label>
            <RsInput
              :model-value="editingCol.comment"
              size="sm"
              :placeholder="t('modules.dameng.design.colCommentPh')"
              @update:model-value="
                updateColSideField(editingCol!.__rowKey, 'comment', String($event ?? ''))
              "
            />
          </div>
          <p class="nm-dameng-design__hint">{{ t('modules.dameng.design.gridEditHint') }}</p>
        </div>
      </template>
      <template v-else-if="activeSection === 'indexes' && editingIdx">
        <div class="nm-dameng-design__editor-title">{{ t('modules.dameng.design.editIndex') }}</div>
        <div class="nm-dameng-design__form">
          <div class="nm-dameng-design__field">
            <label>{{ t('modules.dameng.design.idxName') }}</label>
            <RsInput
              :model-value="editingIdx.name"
              size="sm"
              :disabled="editingIdx.primary"
              :placeholder="t('modules.dameng.design.idxNamePh')"
              @update:model-value="
          ...
          <div class="nm-dameng-design__field">
            <label>{{ t('modules.dameng.design.idxKind') }}</label>
            <RsInput
              :model-value="
                editingIdx.primary
                  ? t('modules.dameng.design.idxKindPrimary')
          ...
          <div class="nm-dameng-design__field">
            <label>{{ t('modules.dameng.design.idxMethod') }}</label>
            <RsSelect
      ...
      <template v-else-if="activeSection === 'foreignKeys' && editingFk">
        <div class="nm-dameng-design__editor-title">{{ t('modules.dameng.design.editForeignKey') }}</div>
        <div class="nm-dameng-design__form">
          <div class="nm-dameng-design__field">
            <label>{{ t('modules.dameng.design.fkRefSchema') }}</label>
            <RsSelect
          ...
          <div class="nm-dameng-design__field">
            <label>{{ t('modules.dameng.design.fkOnDelete') }}</label>
            <RsSelect
          ...
          <div class="nm-dameng-design__field">
            <label>{{ t('modules.dameng.design.fkOnUpdate') }}</label>
            <RsSelect
      ...
      <template v-else-if="activeSection === 'checks' && editingChk">
        <div class="nm-dameng-design__editor-title">{{ t('modules.dameng.design.editCheck') }}</div>
        <div class="nm-dameng-design__form">
          <div class="nm-dameng-design__field">
            <label>{{ t('modules.dameng.design.chkName') }}</label>
            <RsInput
          ...
          <div class="nm-dameng-design__field">
            <label>{{ t('modules.dameng.design.chkExpression') }}</label>
            <RsInput
              :model-value="editingChk.expression"
              size="sm"
              :placeholder="t('modules.dameng.design.chkExpressionPh')"
              @update:model-value="
                checks = checks.map((c) =>
                  c.__rowKey === editingChk!.__rowKey
      ...
      <div v-else class="nm-dameng-design__editor-empty">
        <RsEmpty
          fill
          radius="none"
          icon-radius="none"
          :description="t('modules.dameng.design.selectRow')"
        />
      </div>
    </template>
  </TableDesignShell>
...
<style scoped>
.nm-dameng-design__meta-row {
  display: flex;
  align-items: center;
  gap: 6px;
}
.nm-dameng-design__meta-row--full {
  flex: 1;
  min-width: 240px;
}
.nm-dameng-design__meta-label {
  font-size: 12px;
  color: var(--rs-fg-muted);
  white-space: nowrap;
  min-width: 60px;
}
.nm-dameng-design__meta-readonly {
  font-size: 12px;
  font-weight: 500;
  min-width: 80px;
}
.nm-dameng-design__grid {
  flex: 1;
  min-height: 0;
}
.nm-dameng-design__editor-title {
  font-size: 11px;
  font-weight: 600;
  color: var(--rs-fg-muted);
...
.nm-dameng-design__editor-empty {
  flex: 1;
  display: flex;
  align-items: center;
  justify-content: center;
}
.nm-dameng-design__form {
  display: flex;
  flex-direction: column;
  gap: 8px;
}
.nm-dameng-design__field {
  display: flex;
  flex-direction: column;
  gap: 3px;
}
.nm-dameng-design__field label {
  font-size: 11px;
  color: var(--rs-fg-muted);
}
.nm-dameng-design__field--check {
  flex-direction: row;
  align-items: center;
  gap: 6px;
}
.nm-dameng-design__hint {
  margin: 8px 0 0;
  font-size: 11px;
  color: var(--rs-fg-muted);
  line-height: 1.4;
