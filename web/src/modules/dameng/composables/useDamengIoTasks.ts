...
import type { DamengIoDoneEvent, DamengIoProgressEvent } from '@/api/types/dameng'

export interface DamengIoTaskLine {
  taskId: string
  phase: string
  message: string
  ok?: boolean
...
export function useDamengIoTasks(prefix = 'dameng.io.') {
  const activeTaskId = ref<string | null>(null)
  const lastMessage = ref('')
  const lastOutputPath = ref<string | undefined>()
  const lines = ref<DamengIoTaskLine[]>([])
  let offEvent: (() => void) | null = null
  let trackedTaskId: string | null = null
  let acceptingEarly = false
  let resolveDone: ((ev: DamengIoDoneEvent) => void) | null = null
  const earlyDones = new Map<string, DamengIoDoneEvent>()

  function pushLine(line: Omit<DamengIoTaskLine, 'at'>): void {
  ...
  function applyDone(done: DamengIoDoneEvent): void {
  ...
  function ensureSubscribed(): void {
    ...
    offEvent = subscribeBridgeEventByPrefix(prefix, (detail) => {
      ...
      const event = detail as DamengIoProgressEvent | DamengIoDoneEvent
      if (event.type.endsWith('.progress')) {
        const progress = event as DamengIoProgressEvent
      ...
      const done = event as DamengIoDoneEvent
      if (trackedTaskId) {
  ...
  function waitForTask(taskId: string): Promise<DamengIoDoneEvent> {
    ensureSubscribed()
