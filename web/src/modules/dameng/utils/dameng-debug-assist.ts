/**
 * 达梦调试辅助：日志点 / `_debug` 草稿（只生成文本，不执行 DDL）。
...
/** 已有调试日志点数量。 */
export function countDamengDebugLogPoints(sql: string): number {
...
export function insertDamengDebugLogPoint(
  ddl: string,
  options?: { label?: string },
): string {
  const src = ddl ?? ''
  const n = countDamengDebugLogPoints(src) + 1
...
export function insertDamengDebugLogPointParamGated(
  ddl: string,
  options?: { label?: string },
): string {
  const src = ddl ?? ''
 ...
 */
export function insertDamengDebugLogPointAtLine(
  ddl: string,
  options: { line: number; label?: string },
): string {
  const src = ddl ?? ''
  if (!src.trim() || options.line < 1) {
    return insertDamengDebugLogPoint(src, options)
  }
  const n = countDamengDebugLogPoints(src) + 1
  const label = (options.label || `point_${n}`).trim() || `point_${n}`
  const block = buildLogBlock(label, sessionGateExpr())
...
export function renameDamengRoutineInDdl(
  ddl: string,
...
export function addDamengRoutineInParam(
  ddl: string,
  kind: 'procedure' | 'function',
  paramDecl: string,
): string {
 ...
 * 不触碰线上对象。
 */
export function buildDamengDebugRoutineDraft(options: {
  ddl: string
  kind: 'procedure' | 'function'
  originalName: string
  draftSuffix?: string
  ...
  s = renameDamengRoutineInDdl(s, options.kind, options.originalName, draftName)
  s = addDamengRoutineInParam(s, options.kind, `${NM_DEBUG_PARAM} INT DEFAULT 0`)
  if (countDamengDebugLogPoints(s) === 0) {
    s = insertDamengDebugLogPointParamGated(s, { label: 'enter' })
  }
...
export function wrapDamengCallWithDebugSession(callSql: string): string {
  const lines = [
    'BEGIN',
