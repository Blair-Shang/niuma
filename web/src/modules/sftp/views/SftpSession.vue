...
import { connectionApi, dialogApi, fsApi, sftpApi } from '@/api'
...
import type { SftpEntry } from '@/api/types/sftp'
...
import { useSftpTransfer } from '@/modules/sftp/composables/useSftpTransfer'
import {
  isPathInside,
  readFtpDragPayload,
...
const { sessionId, acquireSession, reconnectSession } = useSessionLease({
  ...
  onAcquired: async (sid) => {
    transferHub.registerSession({
      sessionId: sid,
      provider: 'sftp',
      label: profile.value?.profileName || profile.value?.hostAddress || 'SFTP',
    })
...
const { tasks, enqueue, cancel, pause, resume, refresh: refreshTransfers } = useSftpTransfer(sessionId)
...
async function trustHostKeyIfRejected(message: string): Promise<boolean> {
  ...
  await sftpApi.hostkeyRemember({ host: info.host, port: info.port })
...
async function silentReconnect(): Promise<void> {
  ...
  if (!sessionId.value) {
    throw new Error(t('modules.sftp.session.connectError'))
  }
}
...
async function connectOrRetry(run: () => Promise<void>): Promise<void> {
  ...
  try {
    ...
    const message = e instanceof Error ? e.message : t('modules.sftp.session.connectError')
    try {
...
async function refreshRemote(): Promise<void> {
  ...
  error.value = null
  try {
    const result = await withRemoteReconnect(() => sftpApi.dirList({
    ...
    error.value = message || t('modules.sftp.session.listError')
  } finally {
    loading.value = false
  }
}
...
async function deleteRemoteEntries(items: FtpPaneEntry[]): Promise<void> {
  ...
  try {
    for (const entry of items) {
      try {
        await withRemoteReconnect(() => sftpApi.entryDelete({
          sessionId: sessionId.value!,
...
async function renameRemoteEntry(entry: FtpPaneEntry): Promise<void> {
  ...
  loading.value = true
  try {
    await withRemoteReconnect(() => sftpApi.entryRename({
      sessionId: sessionId.value!,
...
async function mkdirLocal(): Promise<void> {
  ...
  try {
    ...
    toast.error(e instanceof Error ? e.message : t('modules.sftp.session.listError'))
  } finally {
    localLoading.value = false
  }
}
...
async function mkdirRemote(): Promise<void> {
  ...
  loading.value = true
  try {
    await withRemoteReconnect(() => sftpApi.dirMake({
      sessionId: sessionId.value!,
...
<template>
  <div class="nm-sftp-session">
    <p v-if="error" class="nm-sftp-session__error" role="alert">{{ error }}</p>

    <RsLoading v-if="connecting" class="nm-sftp-session__loading" />

    <div v-else class="nm-sftp-session__body">
      <RsSplitPane
        ref="queueSplitRef"
        v-model:sizes="queueSplitSizes"
        class="nm-sftp-session__queue-split"
      ...
      >
        ...
        <template #queue>
          <div
            class="nm-sftp-session__queue-slot"
            :class="{ 'nm-sftp-session__queue-slot--collapsed': queueUiCollapsed }"
          >
            <TransferQueue
...
.nm-sftp-session__error {
  margin: 0;
  color: var(--rs-danger);
  font-size: var(--rs-font-size-sm);
}

.nm-sftp-session__loading {
  flex: 1;
  display: flex;
  align-items: center;
...
.nm-sftp-session__queue-split {
  flex: 1;
  min-height: 0;
}

.nm-sftp-session__queue-split :deep(.rs-split__pane) {
  min-height: 0;
}

/* 折叠/悬浮时分割线始终可见 */
.nm-sftp-session__queue-split :deep(.rs-split__resizer) {
  opacity: 1;
  background: var(--rs-border-subtle);
}

.nm-sftp-session__queue-split :deep(.rs-split__resizer--handle) {
  background: var(--rs-border-subtle);
}

.nm-sftp-session__queue-slot {
  display: flex;
  flex-direction: column;
  width: 100%;
  height: 100%;
...
.nm-sftp-session__queue-slot--collapsed {
  height: auto;
  justify-content: flex-start;
}

.nm-sftp-session__split {
  height: 100%;
  min-height: 0;
}

.nm-sftp-session__split :deep(.rs-split__pane) {
  min-width: 0;
}
</style>
