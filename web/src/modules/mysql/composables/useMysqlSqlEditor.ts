/**
 * MySQL 查询编辑器：薄包装公共 useSqlQueryEditor，注入 mysql LSP 语言 + 文档绑定。
 *
...
/** 多面板共用：bootstrapMysqlMonaco 内部已幂等 */
let sharedBootstrapped = false
let editorSeq = 0

export function useMysqlSqlEditor(options: {
  sqlText: Ref<string>
  active: () => boolean
  onRun: () => void
  ...
  const editorId = `mysql-editor-${++editorSeq}`
  ...
  async function syncLspAttachment(): Promise<void> {
    ...
    const database = scope?.database?.trim() || scope?.schema?.trim() || undefined
    let detach: (() => void) | null = null
    try {
      detach = await attachMysqlSqlLsp({
        model,
        sessionId,
        editorId,
        database,
