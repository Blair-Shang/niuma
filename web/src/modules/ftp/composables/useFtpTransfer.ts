...
/** 传输队列：全局事件总线 + 按 sessionId 过滤（多 Tab keep-alive 安全）。 */
export function useFtpTransfer(sessionId: Ref<string | null>) {
  const tasks = ref<FtpTransferTask[]>([])
  ...
  function updateTask(taskId: string, patch: Partial<FtpTransferTask>): void {
  ...
  function applyEvent(detail: unknown): void {
    ...
    if (typeof e.type !== 'string' || !e.type.startsWith('ftp.transfer.')) {
      return
    }
    ...
    if (e.type === 'ftp.transfer.progress') {
      const speedBps = Number(e.speedBps ?? 0)
      const patch: Partial<FtpTransferTask> = {
    ...
    if (e.type === 'ftp.transfer.state') {
      updateTask(taskId, {
        state: e.state as FtpTransferState,
  ...
  async function refresh(): Promise<void> {
    ...
    const result = await ftpApi.transferList({ sessionId: sessionId.value })
  ...
  async function enqueue(
    params: Omit<import('@/api/types/ftp').FtpTransferEnqueueParams, 'sessionId'>,
  ): Promise<string> {
    if (!sessionId.value) {
      throw new Error('session not open')
    }
    const result = await ftpApi.transferEnqueue({
  ...
  onMounted(() => {
    offEvent = subscribeBridgeEventByPrefix('ftp.transfer.', applyEvent)
  })
