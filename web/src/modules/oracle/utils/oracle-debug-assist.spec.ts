...
describe('insertOracleDebugLogPoint', () => {
  it('inserts DBMS_OUTPUT gate after BEGIN', () => {
    const out = insertOracleDebugLogPoint(PROC)
    expect(countOracleDebugLogPoints(out)).toBe(1)
  ...
  it('inserts after a chosen line', () => {
    const out = insertOracleDebugLogPointAtLine(PROC, { line: 4, label: 'mid' })
  ...
  it('keeps label inside the comment (valid SQL)', () => {
    const out = insertOracleDebugLogPoint(FUNC, { label: 'point_1' })
    expect(out).toContain('/* nm-debug-point: point_1 */')
    expect(out).not.toMatch(/\*\/\s*point_1\b/)
  })
})

describe('buildOracleDebugRoutineDraft', () => {
  it('renames, adds p_nm_debug, and injects enter point', () => {
    const { ddl, draftName } = buildOracleDebugRoutineDraft({
      ddl: PROC,
      kind: 'procedure',
      originalName: 'SP_MIX',
    })
...
describe('rename / add param helpers', () => {
  it('renames qualified routine', () => {
    expect(renameOracleRoutineInDdl(PROC, 'procedure', 'SP_MIX', 'SP_MIX_debug')).toContain(
      '"SP_MIX_debug"',
    )
  })
  ...
  it('appends IN param to empty and non-empty lists', () => {
    ...
    expect(addOracleRoutineInParam(empty, 'procedure', 'p_nm_debug INT DEFAULT 0')).toContain(
      '(p_nm_debug INT DEFAULT 0)',
    )
    expect(addOracleRoutineInParam(PROC, 'procedure', 'p_nm_debug INT DEFAULT 0')).toContain(
      'A INT, p_nm_debug INT DEFAULT 0',
    )
  })
})

describe('wrapOracleCallWithDebugSession', () => {
  it('enables DBMS_OUTPUT and CLIENT_INFO', () => {
    const wrapped = wrapOracleCallWithDebugSession('SELECT 1 FROM DUAL;\n')
