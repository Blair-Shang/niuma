/**
 * SQLite 查询编辑器：Bridge LSP（sqliteparser）+ 静默回退。
 */
...
import {
  attachSqliteSqlLsp,
  bootstrapSqliteMonaco,
} from '@/modules/sqlite/monaco-bootstrap'
...
export function useSqliteSqlEditor(options: {
  ...
  const editorId = `sqlite-editor-${++editorSeq}`
  ...
  async function syncLspAttachment(): Promise<void> {
    ...
    const database = scope?.database?.trim() || scope?.schema?.trim() || 'main'
    let detach: (() => void) | null = null
    try {
      detach = await attachSqliteSqlLsp({
        model,
        sessionId,
        editorId,
        database,
