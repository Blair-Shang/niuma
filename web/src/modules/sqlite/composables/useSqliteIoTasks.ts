...
import type { SqliteIoDoneEvent, SqliteIoProgressEvent } from '@/api/types/sqlite'

export interface SqliteIoTaskLine {
  taskId: string
  phase: string
  message: string
  ok?: boolean
...
export function useSqliteIoTasks(prefix = 'sqlite.io.') {
  const activeTaskId = ref<string | null>(null)
  const lastMessage = ref('')
  const lastOutputPath = ref<string | undefined>()
  const lines = ref<SqliteIoTaskLine[]>([])
  let offEvent: (() => void) | null = null
  let trackedTaskId: string | null = null
  let acceptingEarly = false
  let resolveDone: ((ev: SqliteIoDoneEvent) => void) | null = null
  const earlyDones = new Map<string, SqliteIoDoneEvent>()

  function pushLine(line: Omit<SqliteIoTaskLine, 'at'>): void {
  ...
  function applyDone(done: SqliteIoDoneEvent): void {
  ...
  function ensureSubscribed(): void {
    ...
    offEvent = subscribeBridgeEventByPrefix(prefix, (detail) => {
      ...
      const event = detail as SqliteIoProgressEvent | SqliteIoDoneEvent
      if (event.type.endsWith('.progress')) {
        const progress = event as SqliteIoProgressEvent
      ...
      const done = event as SqliteIoDoneEvent
      if (trackedTaskId) {
  ...
  function waitForTask(taskId: string): Promise<SqliteIoDoneEvent> {
    ensureSubscribed()
