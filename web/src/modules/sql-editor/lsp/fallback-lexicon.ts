...
/** 官方 PostgreSQL：服务端不可用时的着色兜底（不含金仓/Oracle 兼容函数）。 */
export const FALLBACK_POSTGRES_LEXICON: SqlLexicon = {
  keywords: [
    'SELECT', 'FROM', 'WHERE', 'INSERT', 'INTO', 'UPDATE', 'DELETE', 'CREATE', 'OR', 'REPLACE',
    'ALTER', 'DROP', 'TABLE', 'VIEW', 'MATERIALIZED', 'INDEX', 'JOIN', 'LEFT', 'RIGHT', 'INNER',
    'OUTER', 'FULL', 'CROSS', 'ON', 'AND', 'OR', 'NOT', 'NULL', 'AS', 'ORDER', 'BY', 'GROUP',
    ...
    'PARTITION', 'FILTER', 'LATERAL', 'CASCADE', 'RESTRICT', 'PRIMARY', 'KEY', 'FOREIGN',
    'REFERENCES', 'UNIQUE', 'CHECK', 'DEFAULT', 'CONSTRAINT', 'TEMPORARY', 'TEMP', 'UNLOGGED',
    'GENERATED', 'ALWAYS', 'IDENTITY', 'LISTEN', 'NOTIFY', 'UNLISTEN',
  ],
  functions: [
    'CURRENT_DATE', 'CURRENT_TIME', 'CURRENT_TIMESTAMP', 'CURRENT_USER', 'CURRENT_SCHEMA',
    'CURRENT_CATALOG', 'SESSION_USER', 'LOCALTIME', 'LOCALTIMESTAMP', 'NOW', 'CLOCK_TIMESTAMP',
    'STATEMENT_TIMESTAMP', 'TRANSACTION_TIMESTAMP', 'COALESCE', 'NULLIF', 'GREATEST', 'LEAST',
    'TO_CHAR', 'TO_DATE', 'TO_NUMBER', 'TO_TIMESTAMP', 'CAST', 'CONVERT', 'SUBSTRING', 'SUBSTR',
    'OVERLAY', 'LENGTH', 'CHAR_LENGTH', 'CHARACTER_LENGTH', 'OCTET_LENGTH', 'LOWER', 'UPPER',
    'INITCAP', 'TRIM', 'LTRIM', 'RTRIM', 'BTRIM', 'REPLACE', 'LPAD', 'RPAD', 'CONCAT', 'CONCAT_WS',
    'FORMAT', 'ABS', 'CEIL', 'CEILING', 'FLOOR', 'ROUND', 'TRUNC', 'MOD', 'POWER', 'SQRT', 'SIGN',
    'EXTRACT', 'DATE_PART', 'DATE_TRUNC', 'AGE', 'JUSTIFY_INTERVAL', 'COUNT', 'SUM', 'AVG',
    'MAX', 'MIN', 'ROW_NUMBER', 'RANK', 'DENSE_RANK', 'NTILE', 'LAG', 'LEAD', 'FIRST_VALUE',
    'LAST_VALUE', 'STRING_AGG', 'ARRAY_AGG', 'JSON_AGG', 'JSONB_AGG', 'JSON_OBJECT_AGG',
    'TO_JSON', 'TO_JSONB', 'JSONB_BUILD_OBJECT', 'JSONB_BUILD_ARRAY', 'JSONB_SET',
    'CURRENT_DATABASE', 'CURRENT_SETTING', 'SET_CONFIG', 'VERSION', 'PG_TYPEOF',
    'NEXTVAL', 'CURRVAL', 'SETVAL', 'GENERATE_SERIES', 'UUID_GENERATE_V4', 'GEN_RANDOM_UUID',
    'MD5', 'SHA256', 'ENCODE', 'DECODE', 'PG_SIZE_PRETTY', 'ARRAY_TO_STRING', 'STRING_TO_ARRAY',
  ],
}
...
/** Oracle：服务端不可用时的着色兜底（与 dameng 接近的 PL/SQL 子集）。 */
export const FALLBACK_ORACLE_LEXICON: SqlLexicon = {
  keywords: [
    ...
    'GROUP', 'HAVING', 'WITH', 'PIVOT', 'UNPIVOT', 'CROSS', 'APPLY', 'OUTER', 'INNER', 'LEFT',
    'RIGHT', 'FULL', 'JOIN', 'NATURAL', 'CONNECT', 'START', 'PRIOR', 'NOCYCLE', 'LEVEL', 'OVER',
    'PARTITION', 'RANGE', 'ROWS', 'UNBOUNDED', 'PRECEDING', 'FOLLOWING', 'CURRENT', 'ROW',
    'FETCH', 'NEXT', 'ONLY', 'PERCENT', 'TIES', 'OFFSET', 'ROWNUM', 'CREATE', 'OR', 'REPLACE',
    'ALTER', 'DROP', 'TRUNCATE', 'COMMENT', 'TABLE', 'VIEW', 'MATERIALIZED', 'INDEX', 'SEQUENCE',
    'SYNONYM', 'TRIGGER', 'TYPE', 'BODY', 'PACKAGE', 'PROCEDURE', 'FUNCTION', 'SCHEMA',
    'TABLESPACE', 'CONSTRAINT', 'PRIMARY', 'KEY', 'FOREIGN', 'REFERENCES', 'UNIQUE', 'CHECK',
    'DEFAULT', 'ENABLE', 'DISABLE', 'CASCADE', 'EDITIONABLE', 'NONEDITIONABLE', 'GLOBAL',
    'TEMPORARY', 'COMMIT', 'PRESERVE', 'BEGIN', 'DECLARE', 'EXCEPTION', 'RAISE', 'PRAGMA',
    'RETURN', 'RETURNING', 'BULK', 'COLLECT', 'FORALL', 'LOOP', 'WHILE', 'FOR', 'REVERSE',
    'EXIT', 'CONTINUE', 'GOTO', 'IF', 'ELSIF', 'CURSOR', 'OPEN', 'CLOSE', 'OUT', 'INOUT',
    'NOCOPY', 'DETERMINISTIC', 'PIPELINED', 'AUTHID', 'CURRENT_USER', 'DEFINER', 'EXECUTE',
    'IMMEDIATE', 'CALL', 'ROLLBACK', 'SAVEPOINT', 'GRANT', 'REVOKE', 'PUBLIC', 'BEFORE',
    'AFTER', 'INSTEAD', 'EACH', 'REFERENCING', 'OLD', 'NEW', 'VARCHAR2', 'NVARCHAR2', 'CHAR',
    'NUMBER', 'DATE', 'TIMESTAMP', 'INTERVAL', 'RAW', 'BLOB', 'CLOB', 'NCLOB', 'ROWID', 'JSON',
    'BOOLEAN', 'DUAL', 'TRUE', 'FALSE', 'EXPLAIN', 'PLAN', 'LISTAGG', 'WITHIN', 'KEEP',
  ],
  functions: [
    'NVL', 'NVL2', 'DECODE', 'COALESCE', 'NULLIF', 'GREATEST', 'LEAST', 'TO_CHAR', 'TO_DATE',
    'TO_NUMBER', 'TO_TIMESTAMP', 'TO_TIMESTAMP_TZ', 'CAST', 'CONVERT', 'ASCIISTR', 'UNISTR',
    'CHARTOROWID', 'ROWIDTOCHAR', 'RAWTOHEX', 'HEXTORAW', 'SUBSTR', 'SUBSTRB', 'INSTR', 'INSTRB',
    'LENGTH', 'LENGTHB', 'LOWER', 'UPPER', 'INITCAP', 'TRIM', 'LTRIM', 'RTRIM', 'REPLACE',
    'TRANSLATE', 'LPAD', 'RPAD', 'CONCAT', 'CHR', 'ASCII', 'SOUNDEX', 'REGEXP_LIKE',
    'REGEXP_SUBSTR', 'REGEXP_REPLACE', 'REGEXP_INSTR', 'REGEXP_COUNT', 'ABS', 'CEIL', 'FLOOR',
    'ROUND', 'TRUNC', 'MOD', 'POWER', 'SQRT', 'SIGN', 'EXP', 'LN', 'LOG', 'BITAND',
    'WIDTH_BUCKET', 'SYSDATE', 'SYSTIMESTAMP', 'CURRENT_DATE', 'CURRENT_TIMESTAMP',
    'LOCALTIMESTAMP', 'DBTIMEZONE', 'SESSIONTIMEZONE', 'ADD_MONTHS', 'MONTHS_BETWEEN',
    'LAST_DAY', 'NEXT_DAY', 'EXTRACT', 'NUMTODSINTERVAL', 'NUMTOYMINTERVAL', 'FROM_TZ',
    'TZ_OFFSET', 'COUNT', 'SUM', 'AVG', 'MAX', 'MIN', 'STDDEV', 'VARIANCE', 'MEDIAN', 'LISTAGG',
    'XMLAGG', 'ROW_NUMBER', 'RANK', 'DENSE_RANK', 'NTILE', 'LAG', 'LEAD', 'FIRST_VALUE',
    'LAST_VALUE', 'NTH_VALUE', 'RATIO_TO_REPORT', 'CUME_DIST', 'PERCENT_RANK', 'USER', 'UID',
    'SYS_CONTEXT', 'USERENV', 'SYS_GUID', 'ORA_HASH', 'EMPTY_CLOB', 'EMPTY_BLOB', 'JSON_VALUE',
    'JSON_QUERY', 'JSON_TABLE', 'JSON_OBJECT', 'JSON_ARRAY', 'XMLTYPE', 'EXISTSNODE',
    'EXTRACTVALUE',
  ],
}
