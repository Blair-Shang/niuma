...
/** SQL Server：服务端不可用时的着色兜底（子集；完整词表见 sqlserverparser）。 */
export const FALLBACK_SQLSERVER_LEXICON: SqlLexicon = {
  keywords: [
    'SELECT', 'FROM', 'WHERE', 'HAVING', 'GROUP', 'ORDER', 'BY', 'TOP', 'OFFSET', 'FETCH', 'NEXT',
    'ROWS', 'ONLY', 'INSERT', 'INTO', 'VALUES', 'UPDATE', 'SET', 'DELETE', 'MERGE', 'OUTPUT',
    'CREATE', 'ALTER', 'DROP', 'TRUNCATE', 'TABLE', 'VIEW', 'INDEX', 'PROCEDURE', 'FUNCTION',
    'TRIGGER', 'SCHEMA', 'DATABASE', 'BEGIN', 'END', 'TRAN', 'TRANSACTION', 'COMMIT', 'ROLLBACK',
    'DECLARE', 'IF', 'ELSE', 'WHILE', 'RETURN', 'TRY', 'CATCH', 'THROW', 'GOTO', 'WAITFOR',
    'JOIN', 'INNER', 'LEFT', 'RIGHT', 'FULL', 'OUTER', 'CROSS', 'APPLY', 'ON', 'USING',
    'AND', 'OR', 'NOT', 'NULL', 'AS', 'DISTINCT', 'UNION', 'ALL', 'EXCEPT', 'INTERSECT',
    'CASE', 'WHEN', 'THEN', 'ELSE', 'IN', 'EXISTS', 'BETWEEN', 'LIKE', 'IS', 'WITH', 'NOLOCK',
    'GO', 'USE', 'EXEC', 'EXECUTE', 'PRINT', 'RAISERROR', 'IDENTITY', 'PRIMARY', 'KEY',
    'FOREIGN', 'REFERENCES', 'CONSTRAINT', 'DEFAULT', 'CHECK', 'UNIQUE', 'CLUSTERED',
    'NONCLUSTERED', 'INCLUDE', 'OVER', 'PARTITION', 'ROW_NUMBER', 'RANK', 'DENSE_RANK',
  ],
  functions: [
    'GETDATE', 'GETUTCDATE', 'SYSDATETIME', 'SYSUTCDATETIME', 'DATEADD', 'DATEDIFF', 'DATENAME',
    'DATEPART', 'YEAR', 'MONTH', 'DAY', 'CONVERT', 'CAST', 'TRY_CONVERT', 'TRY_CAST',
    'ISNULL', 'NULLIF', 'COALESCE', 'IIF', 'CHOOSE', 'LEN', 'LEFT', 'RIGHT', 'SUBSTRING',
    'REPLACE', 'STUFF', 'CHARINDEX', 'PATINDEX', 'LOWER', 'UPPER', 'LTRIM', 'RTRIM', 'TRIM',
    'CONCAT', 'FORMAT', 'ABS', 'CEILING', 'FLOOR', 'ROUND', 'POWER', 'SQRT', 'SIGN',
    'COUNT', 'SUM', 'AVG', 'MIN', 'MAX', 'STDEV', 'VAR', 'NEWID', 'NEWSEQUENTIALID',
    'OBJECT_ID', 'OBJECT_NAME', 'DB_NAME', 'DB_ID', 'SCHEMA_NAME', 'USER_NAME', 'SUSER_SNAME',
    'SCOPE_IDENTITY', 'IDENT_CURRENT', 'ROWCOUNT_BIG', 'ERROR_MESSAGE', 'ERROR_NUMBER',
