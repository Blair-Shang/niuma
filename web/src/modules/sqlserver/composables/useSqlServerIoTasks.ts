...
import type { SqlServerIoDoneEvent, SqlServerIoProgressEvent } from '@/api/types/sqlserver'

export interface SqlServerIoTaskLine {
  taskId: string
  phase: string
  message: string
  ok?: boolean
...
export function useSqlServerIoTasks(prefix = 'sqlserver.io.') {
  const activeTaskId = ref<string | null>(null)
  const lastMessage = ref('')
  const lastOutputPath = ref<string | undefined>()
  const lines = ref<SqlServerIoTaskLine[]>([])
  let offEvent: (() => void) | null = null
  let trackedTaskId: string | null = null
  let acceptingEarly = false
  let resolveDone: ((ev: SqlServerIoDoneEvent) => void) | null = null
  const earlyDones = new Map<string, SqlServerIoDoneEvent>()

  function pushLine(line: Omit<SqlServerIoTaskLine, 'at'>): void {
  ...
  function applyDone(done: SqlServerIoDoneEvent): void {
  ...
  function ensureSubscribed(): void {
    ...
    offEvent = subscribeBridgeEventByPrefix(prefix, (detail) => {
      ...
      const event = detail as SqlServerIoProgressEvent | SqlServerIoDoneEvent
      if (event.type.endsWith('.progress')) {
        const progress = event as SqlServerIoProgressEvent
      ...
      const done = event as SqlServerIoDoneEvent
      if (trackedTaskId) {
  ...
  function waitForTask(taskId: string): Promise<SqlServerIoDoneEvent> {
    ensureSubscribed()
    // 先挂 resolve，再消费 early done，避免 done 落在二者之间导致 Promise 永不结束、UI 一直「运行中」
