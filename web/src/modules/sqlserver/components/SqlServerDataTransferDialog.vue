...
} from '@/modules/sqlserver/utils/csv-header'
...
const windowTitle = computed(() => task.value?.title ?? t('modules.sqlserver.io.exportTitle'))
const scopeLabel = computed(() => {
  const scope = ctx.value
  if (!scope?.database || !scope.schema || !scope.table) return '—'
  return `${scope.database}.${scope.schema}.${scope.table}`
})
...
const shellLabels = computed(
  (): DataTransferShellLabels => ({
    ...
    confirm: isExport.value ? t('modules.sqlserver.io.export') : t('modules.sqlserver.io.import'),
  }),
)
...
const delimiterOptions = computed<RsSelectOptions>(() => [
  { value: ',', label: t('modules.sqlserver.io.delimiterComma') },
  { value: '\t', label: t('modules.sqlserver.io.delimiterTab') },
  { value: ';', label: t('modules.sqlserver.io.delimiterSemicolon') },
  { value: '|', label: t('modules.sqlserver.io.delimiterPipe') },
])

const targetOptions = computed<RsSelectOptions>(() => [
  { value: '', label: t('modules.sqlserver.io.columnMapSkip') },
...
async function loadTableColumns(): Promise<void> {
  ...
  try {
    const result = await sqlserverApi.metaColumns({
...
async function loadColumnMapFromFile(): Promise<void> {
  ...
  try {
    ...
    if (sources.length === 0) {
      resetMap()
      toast.error(t('modules.sqlserver.io.columnMapEmpty'))
      return
    }
  ...
  try {
    if (current.kind === 'export_csv') {
      const result = await dialogApi.saveFile({
        title: t('modules.sqlserver.io.browseExportTitle'),
      ...
      const result = await dialogApi.openFile({
        title: t('modules.sqlserver.io.browseImportTitle'),
        accept: ['.csv'],
      })
...
async function onConfirm(): Promise<void> {
  const scope = ctx.value
  if (!scope?.database || !scope?.schema || !scope.table || !canConfirm.value) return
  ...
  try {
    ...
    const result = isExport.value
      ? await sqlserverApi.ioExportCsv({
          profileId: scope.profileId,
          sessionId,
          database: scope.database,
          schema: scope.schema,
      ...
      : await sqlserverApi.ioImportCsv({
          profileId: scope.profileId,
          sessionId,
          database: scope.database,
          schema: scope.schema,
    ...
    toast.success(isExport.value ? t('modules.sqlserver.io.exportDone') : t('modules.sqlserver.io.importDone'))
    if (isExport.value) {
  ...
  >
    <DataTransferPanel :labels="panelLabels" :lines="lines" :busy="busy">
      <DataTransferSection :title="t('modules.sqlserver.io.sectionTarget')">
        <div class="nm-sqlserver-dt__scope" :title="scopeLabel">{{ scopeLabel }}</div>
      </DataTransferSection>
      ...
      <DataTransferSection :title="t('modules.sqlserver.io.sectionFormat')">
        <div class="nm-sqlserver-dt__grid">
          <div class="nm-sqlserver-dt__field">
            <RsLabel>{{ t('modules.sqlserver.io.delimiter') }}</RsLabel>
            <RsSelect v-model="delimiter" :options="delimiterOptions" :disabled="busy" />
          </div>
          <div class="nm-sqlserver-dt__field">
            <RsLabel>{{ t('modules.sqlserver.io.nullString') }}</RsLabel>
            <RsInput
              v-model="nullString"
              :disabled="busy"
              :placeholder="t('modules.sqlserver.io.nullStringPh')"
            />
          </div>
        </div>
        <div class="nm-sqlserver-dt__checks">
          <DataTransferCheck
            v-model="header"
            variant="chip"
            :label="t('modules.sqlserver.io.header')"
            :disabled="busy"
          />
          <DataTransferCheck
            v-if="!isExport"
            v-model="truncate"
            variant="chip"
            :label="t('modules.sqlserver.io.truncateFirst')"
            :disabled="busy"
          />
        </div>
      </DataTransferSection>
      ...
      <DataTransferSection
        v-if="!isExport"
        :title="t('modules.sqlserver.io.columnMapTitle')"
      >
        <template #head>
          <RsButton
            size="sm"
          ...
          >
            {{ t('modules.sqlserver.io.columnMapLoad') }}
          </RsButton>
        </template>
        <p class="nm-sqlserver-dt__map-hint">{{ t('modules.sqlserver.io.columnMapHint') }}</p>
        <p v-if="sourceColumns.length" class="nm-sqlserver-dt__map-meta">
          {{ t('modules.sqlserver.io.columnMapMapped', { n: mappedCount, total: sourceColumns.length }) }}
        </p>
        <div v-if="sourceColumns.length" class="nm-sqlserver-dt__map">
          <div class="nm-sqlserver-dt__map-head">
            <span>{{ t('modules.sqlserver.io.columnMapSource') }}</span>
            <span>{{ t('modules.sqlserver.io.columnMapTarget') }}</span>
          </div>
          <div
            v-for="src in sourceColumns"
            :key="src"
            class="nm-sqlserver-dt__map-row"
          >
            <span class="nm-sqlserver-dt__map-src" :title="src">{{ src }}</span>
            <RsSelect
        ...
        <p v-else class="nm-sqlserver-dt__map-empty">{{ t('modules.sqlserver.io.columnMapNeedFile') }}</p>
      </DataTransferSection>

      <template #note>
        {{
          isExport ? t('modules.sqlserver.io.csvExportHint') : t('modules.sqlserver.io.csvImportHint')
        }}
      </template>
    </DataTransferPanel>
  </DataTransferShell>
...
<style scoped>
.nm-sqlserver-dt__scope {
  font-size: var(--rs-font-size-sm, 13px);
  line-height: 1.35;
  word-break: break-all;
}

.nm-sqlserver-dt__grid {
  display: grid;
...
.nm-sqlserver-dt__field {
  display: flex;
  flex-direction: column;
  gap: 4px;
}

.nm-sqlserver-dt__checks {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
  margin-top: 4px;
...
.nm-sqlserver-dt__map-hint,
.nm-sqlserver-dt__map-meta,
.nm-sqlserver-dt__map-empty {
  margin: 0;
...
.nm-sqlserver-dt__map-head,
.nm-sqlserver-dt__map-row {
  display: grid;
