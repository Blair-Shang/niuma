...
if [[ "$SKIP_SHELL_BUILD" != "true" ]]; then
  bash "$SCRIPT_DIR/../build/build-shell.sh" --platform macos --arch "$ARCH" --configuration "$CONFIGURATION"
fi

nm_warn "macOS desktop run is not implemented yet."
nm_warn "skip-services=$SKIP_SERVICES, expected binary/app under: $(nm_shell_exe_path "$REPO_ROOT" macos "$ARCH" "$CONFIGURATION")"
