...
| `shared/sign/sign-windows.ps1` | 可选 Authenticode（`CODESIGN_CERT`） |
| `shared/sign/notarize-macos.sh` | 可选 macOS 公证（`NOTARY_*`） |
...
1. 根 `package.json` 的 `version` 已更新
...
6. macOS 打 pkg 需 Xcode CLI；对外分发建议配置 `CODESIGN_IDENTITY` 与 `NOTARY_*`

### 可选签名

```powershell
# Windows（仅 Setup.exe，需证书）
$env:CODESIGN_CERT = "<thumbprint 或证书主题名>"
pnpm pack:win:setup
```

```bash
# macOS 公证（打包脚本内会在配置环境变量后调用 notarytool）
...
- 单一版本源 + Go/Rust/Shell/Web 构建注入
- 各平台安装包内含 `licenses/`（CEF 许可、NOTICES）与 `version.json`
- Windows Inno Setup、Linux/麒麟 zenity `.run`、macOS `.pkg` 向导式安装
- 本机构建校验，禁止跨 OS 打包

## 后续可选增强

- CI 矩阵（各 OS runner 跑 `release:*`）
