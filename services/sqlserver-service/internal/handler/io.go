...
func (d *Dispatcher) resolveTaskConnect(raw json.RawMessage, sessionID, database string) (session.ConnectParams, string, error) {
	...
	if sessionID != "" {
		...
		if err != nil {
			return session.ConnectParams{}, "", fmt.Errorf("sqlserver: session not found: %s", sessionID)
		}
		p := sess.Params
		if database != "" {
...
func (d *Dispatcher) ioExportCsv(ctx context.Context, req Request) Response {
	...
	taskID, err := d.io.ExportCsv(ctx, connect, sessionID, params.Database, schema, params.Table, params.OutputPath, params.CsvOptions)
	if err != nil {
		logOpWarn(MethodIOExportCsv, err, "session", sessionID, "database", params.Database, "schema", schema, "table", params.Table)
		return errorResponse(req.ID, err.Error())
	}
	logOpInfo(MethodIOExportCsv, "session", sessionID, "database", params.Database, "schema", schema, "table", params.Table, "task", taskID)
...
func (d *Dispatcher) ioImportCsv(ctx context.Context, req Request) Response {
	...
	taskID, err := d.io.ImportCsv(ctx, connect, sessionID, params.Database, schema, params.Table, params.InputPath, params.CsvOptions)
	if err != nil {
		logOpWarn(MethodIOImportCsv, err, "session", sessionID, "database", params.Database, "schema", schema, "table", params.Table)
		return errorResponse(req.ID, err.Error())
	}
	logOpInfo(MethodIOImportCsv, "session", sessionID, "database", params.Database, "schema", schema, "table", params.Table, "task", taskID)
	return okResponse(req.ID, map[string]any{"taskId": taskID})
}
