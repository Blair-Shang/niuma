package session

import (
	"context"
	"database/sql"
	"sync"

	"niuma/services/sqlite-service/internal/dialect"
)

type queryCancel struct {
	cancel context.CancelFunc
}

// Session 持有一条 SQLite 连接、在途查询与结果游标。
type Session struct {
	ID      string
	DB      *sql.DB
	Params  ConnectParams
	Dialect *dialect.ServerProfile

	mu         sync.Mutex
	inflight   map[string]*queryCancel
	resultSets map[string]*ResultSet

	autoCommit bool
	inTx       bool
	txBusy     bool
}

// NewSession 创建默认 Auto-commit 的会话（对齐 Navicat / DBeaver / IDEA）。
func NewSession(id string, db *sql.DB, params ConnectParams, profile *dialect.ServerProfile) *Session {
	return &Session{
		ID:         id,
		DB:         db,
		Params:     params,
		Dialect:    profile,
		autoCommit: true,
	}
}

// RegisterQuery 登记可取消查询。
func (s *Session) RegisterQuery(parent context.Context, requestID string) (context.Context, func()) {
	ctx, cancel := context.WithCancel(parent)
	entry := &queryCancel{cancel: cancel}

	s.mu.Lock()
	if s.inflight == nil {
		s.inflight = make(map[string]*queryCancel)
	}
	if prev, ok := s.inflight[requestID]; ok {
		prev.cancel()
	}
	s.inflight[requestID] = entry
	s.mu.Unlock()

	release := func() {
		s.mu.Lock()
		if cur, ok := s.inflight[requestID]; ok && cur == entry {
			delete(s.inflight, requestID)
		}
		s.mu.Unlock()
		cancel()
	}
	return ctx, release
}

// CancelQuery 取消指定或全部在途查询与游标。
func (s *Session) CancelQuery(requestID string) int {
	n := 0
	if requestID != "" {
		s.mu.Lock()
		var toClose []*ResultSet
		for id, rs := range s.resultSets {
			if rs.RequestID == requestID {
				toClose = append(toClose, rs)
				delete(s.resultSets, id)
			}
		}
		entry, ok := s.inflight[requestID]
		if ok {
			delete(s.inflight, requestID)
		}
		s.mu.Unlock()
		for _, rs := range toClose {
			rs.forceClose()
			n++
		}
		if ok {
			entry.cancel()
			n++
		}
		return n
	}

	s.mu.Lock()
	allRS := make([]*ResultSet, 0, len(s.resultSets))
	for id, rs := range s.resultSets {
		allRS = append(allRS, rs)
		delete(s.resultSets, id)
	}
	allInflight := make([]*queryCancel, 0, len(s.inflight))
	for id, entry := range s.inflight {
		allInflight = append(allInflight, entry)
		delete(s.inflight, id)
	}
	s.mu.Unlock()
	for _, rs := range allRS {
		rs.forceClose()
		n++
	}
	for _, entry := range allInflight {
		entry.cancel()
		n++
	}
	return n
}

// Close 关闭连接池、游标与在途查询。
func (s *Session) Close() {
	s.CloseResultSet("")
	s.CancelQuery("")
	s.mu.Lock()
	s.autoCommit = true
	s.inTx = false
	s.txBusy = false
	s.mu.Unlock()
	if s.DB != nil {
		_ = s.DB.Close()
		s.DB = nil
	}
}
