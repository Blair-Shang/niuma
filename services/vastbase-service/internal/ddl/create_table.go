...
func validateIdentPart(name, field string) error {
	...
	if len([]rune(n)) > 63 {
		return fmt.Errorf("vastbase: %s too long (max 63)", field)
	}
	if strings.ContainsRune(n, 0) {
		return fmt.Errorf("vastbase: %s contains NUL", field)
	}
	return nil
}
...
func validateDataType(dt string) error {
	...
	if t == "" {
		return fmt.Errorf("vastbase: dataType required")
	}
	// 拒绝明显注入片段；允许 varchar(255)、timestamp with time zone、double precision 等。
	lower := strings.ToLower(t)
	for _, bad := range []string{";", "--", "/*", "*/", "\n", "\r"} {
		if strings.Contains(lower, bad) {
			return fmt.Errorf("vastbase: dataType contains forbidden characters")
		}
	}
	return nil
}
...
func validateDefaultExpr(expr string) error {
	...
	for _, bad := range []string{";", "--", "/*", "*/"} {
		if strings.Contains(lower, bad) {
			return fmt.Errorf("vastbase: default expression contains forbidden characters")
		}
	}
	return nil
}
...
func PreviewCreateTable(params CreateTableParams) (*CreateTableResult, error) {
	...
	if len(params.Columns) == 0 {
		return nil, fmt.Errorf("vastbase: at least one column required")
	}
	...
	for i, col := range params.Columns {
		...
		if _, ok := seen[key]; ok {
			return nil, fmt.Errorf("vastbase: duplicate column %q", name)
		}
		seen[key] = struct{}{}

		if err := validateDataType(col.DataType); err != nil {
			return nil, fmt.Errorf("vastbase: columns[%d]: %w", i, err)
		}
	...
	for i, idx := range params.Indexes {
		...
		if name == "" {
			return nil, fmt.Errorf("vastbase: indexes[%d].name required", i)
		}
		...
		if err != nil {
			return nil, fmt.Errorf("vastbase: indexes[%d]: %w", i, err)
		}
		unique := ""
		if idx.Unique {
			unique = "UNIQUE "
	...
	for i, ck := range params.Checks {
		...
		if err := validateSQLFragment(expr, "check expression"); err != nil {
			return nil, fmt.Errorf("vastbase: checks[%d]: %w", i, err)
		}
	...
	for i, fk := range params.ForeignKeys {
		...
		if err != nil {
			return nil, fmt.Errorf("vastbase: foreignKeys[%d]: %w", i, err)
		}
		refCols, err := quoteIdentList(fk.RefColumns)
		if err != nil {
			return nil, fmt.Errorf("vastbase: foreignKeys[%d].refColumns: %w", i, err)
		}
		...
		if refTable == "" {
			return nil, fmt.Errorf("vastbase: foreignKeys[%d].refTable required", i)
		}
