...
ORDER BY 1`
	...
	if err != nil {
		return nil, fmt.Errorf("vastbase: list encodings: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&enc); err != nil {
			return nil, fmt.Errorf("vastbase: list encodings scan: %w", err)
		}
...
func listDatabaseCollations(
	...
	// 1) 按编码过滤 pg_collation（PostgreSQL 标准路径）
	const byEncoding = `
SELECT c.collname
...
LIMIT $2`
	...
	// 2) 全量 pg_collation（Vastbase / openGauss 部分版本无 collencoding 映射）
	if len(merged) == 0 {
		const allCollations = `
SELECT c.collname
FROM pg_catalog.pg_collation c
...
LIMIT $1`
	...
	// 3) 已有库的 datcollate / datctype（实例真实可用 locale）
	const fromDatabases = `
...
func scanStringList(ctx context.Context, pool *pgxpool.Pool, query string, args ...any) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("vastbase: query options: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&value); err != nil {
			return nil, fmt.Errorf("vastbase: scan options: %w", err)
		}
		value = strings.TrimSpace(value)
		if value == "" {
			continue
