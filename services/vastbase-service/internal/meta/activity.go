...
// 使用单参数 current_setting(text)：部分 Vastbase / 旧内核无 current_setting(text, bool)。
...
`).Scan(
	...
	if err != nil {
		return nil, fmt.Errorf("vastbase: instance overview: %w", err)
	}
	return out, nil
}
...
func ListActivity(ctx context.Context, pool *pgxpool.Pool) (*ActivityResult, error) {
	out, err := listActivity(ctx, pool, activitySQLVastbase)
...
// activitySQLVastbase：含 sessionid / query_id（openGauss / Vastbase 常见扩展列）。
const activitySQLVastbase = `
SELECT a.pid::bigint,
...
func listActivity(ctx context.Context, pool *pgxpool.Pool, querySQL string) (*ActivityResult, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("vastbase: list activity: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("vastbase: scan activity: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("vastbase: activity rows: %w", err)
	}
	return out, nil
}
...
`)
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("vastbase: scan lock: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("vastbase: locks rows: %w", err)
	}
	return out, nil
}
...
`)
	if err != nil {
		return nil, fmt.Errorf("vastbase: list locks: %w", err)
	}
	defer rows.Close()
...
func queryBlockingEdges(ctx context.Context, pool *pgxpool.Pool, querySQL string) ([]LockBlockingEdge, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.BlockedPID, &e.BlockingPID); err != nil {
			return nil, fmt.Errorf("vastbase: scan blocking edge: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("vastbase: blocking edges rows: %w", err)
	}
	return out, nil
}
...
// Vastbase / 线程模型下 pid 可能超出 int4，须按 bigint 传递。
...
func backendAction(ctx context.Context, pool *pgxpool.Pool, pid int64, fn string) (*BackendActionResult, error) {
	if pid <= 0 {
		return nil, fmt.Errorf("vastbase: invalid backend pid")
	}
	if fn != "pg_cancel_backend" && fn != "pg_terminate_backend" {
		return nil, fmt.Errorf("vastbase: unknown backend action")
	}
	var self int64
	if err := pool.QueryRow(ctx, `SELECT pg_backend_pid()`).Scan(&self); err != nil {
		return nil, fmt.Errorf("vastbase: read backend pid: %w", err)
	}
	if pid == self {
		return nil, fmt.Errorf("vastbase: cannot act on current backend")
	}

	// 优先 bigint：线程模型 pid 远超 int4；回退经 pg_stat_activity 取同类型 pid 再调用。
	ok, err := callBackendFn(ctx, pool, fn, pid)
	if err != nil {
		return nil, fmt.Errorf("vastbase: backend action: %w", err)
	}
