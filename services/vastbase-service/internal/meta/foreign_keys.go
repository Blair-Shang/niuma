...
type ForeignKeyInfo struct {
	...
	// OnDelete / OnUpdate：NO ACTION | RESTRICT | CASCADE | SET NULL | SET DEFAULT
...
ORDER BY con.conname`
	...
	if err != nil {
		return nil, fmt.Errorf("vastbase: list foreign keys: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("vastbase: foreign keys scan: %w", err)
		}
		if fk.Columns == nil {
			fk.Columns = []string{}
		}
