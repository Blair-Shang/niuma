...
// ensureConnDatabase 在连接上 USE 到目标库（标识符经 QuoteIdent，防注入）。
func (s *Session) ensureConnDatabase(ctx context.Context, conn *sql.Conn, database string) error {
	...
	if !IsSafeDatabaseName(database) {
		return fmt.Errorf("clickhouse: unsafe database name %q", database)
	}
	s.mu.Lock()
	current := s.currentDB
	s.mu.Unlock()
	...
	if _, err := conn.ExecContext(ctx, "USE "+ident); err != nil {
		return fmt.Errorf("clickhouse: use database: %w", err)
	}
	s.mu.Lock()
