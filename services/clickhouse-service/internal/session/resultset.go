...
// 调用方须保证失败/耗尽路径关闭 Conn 与 Rows，避免连接与游标泄漏。
func OpenPagedQuery(
	ctx context.Context,
	sess *Session,
	db *sql.DB,
	...
	if sess == nil {
		return nil, fmt.Errorf("clickhouse: session required for paged query")
	}
	if db == nil {
		return nil, fmt.Errorf("clickhouse: db required")
	}
	...
	if err != nil {
		cleanupFailed()
		return nil, fmt.Errorf("clickhouse: acquire: %w", err)
	}
	...
	if !returnsResultSet(sqlText) {
		...
		if eerr != nil {
			releaseConn()
			cleanupFailed()
			return nil, fmt.Errorf("clickhouse: exec: %w", eerr)
		}
...
func (s *Session) getResultSet(id string) (*ResultSet, error) {
	...
	if !ok {
		return nil, fmt.Errorf("clickhouse: result set not found: %s", id)
	}
	return rs, nil
}
...
func (rs *ResultSet) readPage(limit int) (page [][]any, hasMore bool, truncated bool, err error) {
	...
	defer rs.mu.Unlock()
	if rs.closed {
		return nil, false, false, fmt.Errorf("clickhouse: result set closed")
	}
	...
	if err := rs.rows.Err(); err != nil {
		return nil, false, false, fmt.Errorf("clickhouse: rows: %w", err)
	}
