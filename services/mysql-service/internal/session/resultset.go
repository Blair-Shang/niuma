...
func OpenPagedQuery(
	...
	if sess == nil {
		return nil, fmt.Errorf("mysql: session required for paged query")
	}
	if db == nil {
		return nil, fmt.Errorf("mysql: db required")
	}
...
func (s *Session) getResultSet(id string) (*ResultSet, error) {
	...
	if !ok {
		return nil, fmt.Errorf("mysql: result set not found: %s", id)
	}
	return rs, nil
}
...
func (rs *ResultSet) readPage(limit int) (page [][]any, hasMore bool, truncated bool, err error) {
	...
	defer rs.mu.Unlock()
	if rs.closed {
		return nil, false, false, fmt.Errorf("mysql: result set closed")
	}
	...
	if err := rs.rows.Err(); err != nil {
		return nil, false, false, fmt.Errorf("mysql: rows: %w", err)
	}
