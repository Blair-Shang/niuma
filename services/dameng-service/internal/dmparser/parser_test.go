...
func TestKeywordsNonEmpty(t *testing.T) {
	...
	if len(p.Keywords()) < 20 {
		t.Fatal("expected dameng keywords")
	}
}
...
func TestClassify(t *testing.T) {
	...
	for _, c := range cases {
		got := dmparser.Classify(c.sql).String()
		if got != c.kind {
...
func TestCompletionSnippetsNoDelimiter(t *testing.T) {
	...
	for _, sn := range cc.Snippets {
		if strings.Contains(strings.ToUpper(sn.InsertText), "DELIMITER") {
			t.Fatalf("dameng snippet must not use DELIMITER: %s", sn.Label)
		}
		if strings.Contains(sn.InsertText, "`") {
			t.Fatalf("dameng snippet must use double-quote ident, got backtick in %s", sn.Label)
		}
	}
}
...
func TestWorkASTTables(t *testing.T) {
	...
	ast := dmparser.ParseWorkAST(sql, sqllsp.Position{Character: len(sql)})
	if len(ast.Tables) < 2 {
...
func TestCompatOracleLimitHint(t *testing.T) {
	p := dmparser.NewWithCompat(dmparser.CompatOracle)
...
func TestCompatAutoNoMysqlOracleUnion(t *testing.T) {
	p := dmparser.NewWithCompat(dmparser.CompatAuto)
	kws := p.Keywords()
...
func TestCompatMysqlBuiltinsExclusive(t *testing.T) {
	p := dmparser.NewWithCompat(dmparser.CompatMysql)
	if p.BuiltinSignature("IFNULL") == nil {
