...
ORDER BY i.relname`
	...
	if err != nil {
		return nil, fmt.Errorf("kingbase: list indexes: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("kingbase: list indexes scan: %w", err)
		}
		if idx.Columns == nil {
			idx.Columns = []string{}
		}
...
ORDER BY con.conname`
	...
	rows, err := pool.Query(ctx, q, oid)
	if err != nil {
		return nil, fmt.Errorf("kingbase: list constraints: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.Name, &c.Type, &c.Definition); err != nil {
			return nil, fmt.Errorf("kingbase: list constraints scan: %w", err)
		}
...
LIMIT 1`, ref.Schema, ref.Name).Scan(&relkind)
	if err != nil {
		return nil, fmt.Errorf("kingbase: resolve relation: %w", err)
	}
...
WHERE n.nspname = $1 AND c.relname = $2`, ref.Schema, ref.Name).Scan(&ddl)
		if err != nil {
			return nil, fmt.Errorf("kingbase: view ddl: %w", err)
		}
		kw := "VIEW"
		if relkind == "m" {
...
func fetchRoutineDefinition(ctx context.Context, pool *pgxpool.Pool, oid uint32) (string, error) {
	...
	return "", fmt.Errorf("kingbase: pg_get_functiondef: %v (record fallback: %w)", textErr, recordErr)
}
...
WHERE n.nspname = $1 AND p.proname = $2 ORDER BY p.oid`, ref.Schema, ref.Name)
		if queryErr != nil {
			return nil, fmt.Errorf("kingbase: routine source: %w", queryErr)
		}
		defer rows.Close()
		count := 0
		for rows.Next() {
			count++
			if count > 1 {
				return nil, fmt.Errorf("kingbase: routine %s.%s is overloaded; pass oid or args", ref.Schema, ref.Name)
			}
			if err = scanMeta(rows); err != nil {
				return nil, fmt.Errorf("kingbase: routine source: %w", err)
			}
		}
		if err = rows.Err(); err != nil {
			return nil, fmt.Errorf("kingbase: routine source: %w", err)
		...
		if count == 0 {
			return nil, fmt.Errorf("kingbase: routine source: %w", pgx.ErrNoRows)
		}
	}
	if err != nil {
		return nil, fmt.Errorf("kingbase: routine source: %w", err)
