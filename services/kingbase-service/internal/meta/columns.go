// Package meta 提供 KingbaseES 对象元数据查询。
//
...
ORDER BY a.attnum`
	...
	if err != nil {
		return nil, fmt.Errorf("kingbase: list columns: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&col.Ordinal, &col.Name, &col.DataType, &col.Nullable, &def, &comment); err != nil {
			return nil, fmt.Errorf("kingbase: list columns scan: %w", err)
		}
		col.Default = def
		if comment != nil {
			col.Comment = *comment
...
LIMIT 1`
	...
	if err := pool.QueryRow(ctx, q, oid).Scan(&comment); err != nil {
		...
		return "", fmt.Errorf("kingbase: table comment: %w", err)
	}
	if comment.Valid {
