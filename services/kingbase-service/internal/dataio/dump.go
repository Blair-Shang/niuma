...
func dumpSql(ctx context.Context, pool *pgxpool.Pool, taskID string, m *Manager, params DumpParams) error {
	...
	if len(targets) == 0 {
		return fmt.Errorf("kingbase: no objects to dump")
	}
	...
	if err != nil {
		return fmt.Errorf("kingbase: create dump file: %w", err)
	}
	defer f.Close()
...
func writeStructureBlock(ctx context.Context, pool *pgxpool.Pool, w *countingWriter, t dumpTarget) error {
	...
	case "function", "procedure":
		...
		if err != nil {
			return fmt.Errorf("kingbase: routine ddl %s.%s: %w", t.Schema, t.Name, err)
		}
	...
	case "trigger":
		...
		if err != nil {
			return fmt.Errorf("kingbase: trigger ddl %s.%s: %w", t.Schema, t.Name, err)
		}
		block := fmt.Sprintf(
	...
	default:
		...
		if err != nil {
			return fmt.Errorf("kingbase: ddl %s.%s: %w", t.Schema, t.Name, err)
		}
...
func resolveDumpTargets(ctx context.Context, pool *pgxpool.Pool, params DumpParams) ([]dumpTarget, error) {
	...
	if len(relTypes) == 0 &&
		...
		return nil, fmt.Errorf("kingbase: no object types selected")
	}
...
SELECT EXISTS (
  ...
  WHERE d.objid = $1 AND d.deptype = 'e'
	...
	if err != nil {
		return false, fmt.Errorf("kingbase: check extension-owned routine: %w", err)
	}
	return owned, nil
}
...
func writeCopyDataBlock(ctx context.Context, pool *pgxpool.Pool, w *countingWriter, schema, table string) error {
	...
	if err != nil {
		return fmt.Errorf("kingbase: dump data %s: %w", qn, err)
	}
	if _, err := w.Write([]byte("\\.\n\n")); err != nil {
		return err
	}
...
func schemaHasUsage(ctx context.Context, pool *pgxpool.Pool, schema string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("kingbase: check schema usage %s: %w", schema, err)
	}
	return ok, nil
}
...
LIMIT 1`, schema, name).Scan(&ok)
	...
	if err != nil {
		return false, fmt.Errorf("kingbase: check select %s.%s: %w", schema, name, err)
	}
	return ok, nil
}
