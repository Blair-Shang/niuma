...
// kbNativeBuiltins Kingbase 原生常用函数（各兼容模式均可用）。
func kbNativeBuiltins() []builtinFn {
	return []builtinFn{
...
func builtinsForCompat(mode CompatMode) []builtinFn {
	out := append([]builtinFn(nil), kbNativeBuiltins()...)
	switch mode {
	case CompatOracle:
...
// kingbaseBuiltinFunctionNames 内置函数名列表（去重，供 Monarch / lexicon）。
func kingbaseBuiltinFunctionNames(mode CompatMode) []string {
...
func kingbaseBuiltinFunctions(mode CompatMode) []sqllsp.CompletionItem {
	defs := builtinsForCompat(mode)
...
// QuoteIdent 实现 sqllsp.IdentifierQuoter（Kingbase 双引号）。
func (p *Parser) QuoteIdent(name string) string {
	return quoteKingbaseIdent(name)
}
...
func identNeedsQuote(name string) bool {
	...
	// 全小写/全大写无空格的简单名可不引号；混合大小写在Kingbase 默认折叠，保留不强制引号
	return false
}
