...
func quoteIdentList(names []string) (string, error) {
	if len(names) == 0 {
		return "", fmt.Errorf("kingbase: columns required")
	}
	parts := make([]string, 0, len(names))
	for i, n := range names {
		n = strings.TrimSpace(n)
		if n == "" {
			return "", fmt.Errorf("kingbase: columns[%d] empty", i)
		}
...
func validateSQLFragment(expr, field string) error {
	...
	for _, bad := range []string{";", "--", "/*", "*/"} {
		if strings.Contains(lower, bad) {
			return fmt.Errorf("kingbase: %s contains forbidden characters", field)
		}
	}
	return nil
}
...
func normalizeFKAction(raw string) (string, error) {
	...
	return "", fmt.Errorf("kingbase: unsupported FK action %q", raw)
}
...
func normalizeIndexMethod(raw string) (string, error) {
	...
	return "", fmt.Errorf("kingbase: unsupported index method %q", raw)
}
...
func buildConstraintDesignSQL(schema, table string, op DesignOp) (string, error) {
	...
	case DesignDropPrimaryKey:
		name := strings.TrimSpace(op.Name)
		if name == "" {
			return "", fmt.Errorf("kingbase: drop_primary_key requires constraint name")
		}
	...
	case DesignAddIndex:
		...
		name := strings.TrimSpace(op.Name)
		if name == "" {
			return "", fmt.Errorf("kingbase: add_index requires name")
		}
		unique := ""
	...
	case DesignDropIndex:
		name := strings.TrimSpace(op.Name)
		if name == "" {
			return "", fmt.Errorf("kingbase: drop_index requires name")
		}
	...
	case DesignRenameIndex:
		name := strings.TrimSpace(op.Name)
		if name == "" {
			return "", fmt.Errorf("kingbase: rename_index requires name")
		}
	...
	case DesignDropConstraint:
		name := strings.TrimSpace(op.Name)
		if name == "" {
			return "", fmt.Errorf("kingbase: drop_constraint requires name")
		}
		return fmt.Sprintf("ALTER TABLE %s DROP CONSTRAINT IF EXISTS %s", rel, quoteIdent(name)), nil
	...
	case DesignAddForeignKey:
		...
		if err != nil {
			return "", fmt.Errorf("kingbase: refColumns: %w", err)
		}
		...
		if refTable == "" {
			return "", fmt.Errorf("kingbase: add_foreign_key requires refTable")
		}
		name := strings.TrimSpace(op.Name)
		if name == "" {
	...
	default:
		return "", fmt.Errorf("kingbase: unsupported design op %q", op.Op)
	}
}
