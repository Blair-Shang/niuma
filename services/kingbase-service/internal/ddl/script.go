...
// maintenanceDatabaseFallbacks 是金仓/PG 线常见维护库候选（按优先级）。
// 金仓默认初始库多为 TEST，不一定存在 postgres。
var maintenanceDatabaseFallbacks = []string{"TEST", "kingbase", "postgres", templateDatabaseName}
...
type ScriptParams struct {
	...
	// Capabilities 会话方言能力（缺省则按 DefaultKingbase）。
...
func requireSchemaName(schema, name string) error {
	if strings.TrimSpace(schema) == "" || strings.TrimSpace(name) == "" {
		return fmt.Errorf("kingbase: schema and name required")
	}
	return nil
}
...
func requireNewName(newName string) error {
	if strings.TrimSpace(newName) == "" {
		return fmt.Errorf("kingbase: newName required")
	}
	return nil
}
...
func requireDatabaseName(name string) error {
	if strings.TrimSpace(name) == "" {
		return fmt.Errorf("kingbase: database name required")
	}
	return nil
}
...
func IsProtectedSchema(name string) bool {
	...
	switch n {
	case "information_schema", "pg_catalog", "sys", "sys_catalog", "plsql_debug", "perf":
		return true
	}
	return strings.HasPrefix(n, "pg_") || strings.HasPrefix(n, "sys_")
}
...
func buildCreateDatabaseSQL(name string, params ScriptParams) (string, error) {
	...
	if owner != "" && !strings.EqualFold(owner, "CURRENT_USER") {
		if !isSafeDatabaseIdent(owner) {
			return "", fmt.Errorf("kingbase: invalid database owner %q", owner)
		}
	...
	if _, ok := allowedDatabaseEncodings[encoding]; !ok {
		if !isSafeDatabaseLiteral(encoding) {
			return "", fmt.Errorf("kingbase: unsupported database encoding %q", encoding)
		}
	}
	...
	if _, ok := allowedDatabaseTemplates[template]; !ok {
		if !isSafeDatabaseIdent(template) {
			return "", fmt.Errorf("kingbase: unsupported database template %q", template)
		}
	}
...
func buildOwnerClause(owner string) (string, error) {
	...
	if !isSafeDatabaseIdent(owner) {
		return "", fmt.Errorf("kingbase: invalid owner %q", owner)
	}
...
/`, qn),
	...
	case ActionCreateProcedure:
		qn := qualified(schema, name)
		profile := scriptProfile(params.Capabilities)
		if dialect.Has(profile, dialect.CapFuncPlpgsqlDollar) && !dialect.Has(profile, dialect.CapProcPlsqlBare) {
			return &ScriptResult{
				Action: action,
...
END;%s`, qn, slash),
	...
	case ActionRenameDatabase:
		...
		if isProtectedDatabase(params.Name) {
			return nil, fmt.Errorf("kingbase: cannot rename protected database %q", params.Name)
		}
		if err := requireNewName(params.NewName); err != nil {
			return nil, err
		}
	...
	case ActionDropDatabase:
		...
		if isProtectedDatabase(params.Name) {
			return nil, fmt.Errorf("kingbase: cannot drop protected database %q", params.Name)
		}
		return &ScriptResult{
			Action:  action,
	...
	case ActionRenameSchema:
		...
		if IsProtectedSchema(params.Name) {
			return nil, fmt.Errorf("kingbase: cannot rename protected schema %q", params.Name)
		}
		if err := requireNewName(params.NewName); err != nil {
			return nil, err
		}
	...
	case ActionDropSchema:
		...
		if IsProtectedSchema(params.Name) {
			return nil, fmt.Errorf("kingbase: cannot drop protected schema %q", params.Name)
		}
		return &ScriptResult{
			Action:  action,
	...
	case ActionAlterSchemaOwner:
		...
		if IsProtectedSchema(params.Name) {
			return nil, fmt.Errorf("kingbase: cannot alter owner of protected schema %q", params.Name)
		}
		ownerSQL, err := buildOwnerClause(params.Owner)
		if err != nil {
			return nil, err
	...
	default:
		return nil, fmt.Errorf("kingbase: unsupported ddl action %q", action)
	}
}
...
func Exec(ctx context.Context, pool *pgxpool.Pool, params ExecParams) (*ExecResult, error) {
	...
	if !script.Danger && strings.TrimSpace(params.Action) != ActionCreateDatabase && strings.TrimSpace(params.Action) != ActionCreateSchema {
		return nil, fmt.Errorf("kingbase: action %q is not executable via ddl.exec; open query instead", params.Action)
	}

	start := time.Now()
	tag, err := pool.Exec(ctx, script.SQL)
	if err != nil {
		return nil, fmt.Errorf("kingbase: ddl exec: %w", err)
	}
	return &ExecResult{
		Action:     params.Action,
