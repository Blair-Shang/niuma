// Package session 管理 KingbaseES（PG 线协议）连接、会话池与 SQL 查询执行。
//
// 连接参数与 Web connection_options 对齐，详见 docs/31-kingbase-module.md §4。
...
const (
	// DefaultPort 是 KingbaseES 默认端口。
	DefaultPort = 54321

	defaultTimeoutSeconds = 10
	defaultDatabase       = "TEST"
	defaultSSLMode        = "prefer"
	defaultAppName        = "niuma-kingbase"
)
...
// DatabaseOrDefault 返回配置的库名；空则回退到 TEST。
...
func PrepareDialParams(ctx context.Context, params ConnectParams) (ConnectParams, func(), error) {
	...
	if err != nil {
		return ConnectParams{}, nil, fmt.Errorf("kingbase: ssh tunnel: %w", err)
	}
	p.HostAddress = host
	p.PortNumber = port
...
func ConnectWithRuntime(ctx context.Context, params ConnectParams, rt ConnectRuntime) (*pgxpool.Pool, func(), error) {
	...
	if p.Options.Tunnel != nil && p.Options.Tunnel.Enabled() {
		...
		if err != nil {
			return nil, nil, fmt.Errorf("kingbase: ssh tunnel: %w", err)
		}
		p.HostAddress = host
		p.PortNumber = port
		tunnelStop = stop
	...
	if err != nil {
		...
		return nil, nil, fmt.Errorf("kingbase: connect: %w", err)
	}
	...
	if err := pool.Ping(pingCtx); err != nil {
		...
		return nil, nil, fmt.Errorf("kingbase: ping: %w", err)
	}
...
func buildPoolConfig(params ConnectParams) (*pgxpool.Config, error) {
	...
	if host == "" {
		return nil, fmt.Errorf("kingbase: host address required")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("kingbase: parse config: %w", err)
	}
	...
	cfg.AfterConnect = func(ctx context.Context, conn *pgx.Conn) error {
		if searchPath != "" {
			if _, err := conn.Exec(ctx, "SELECT set_config('search_path', $1, false)", searchPath); err != nil {
				return fmt.Errorf("kingbase: set search_path: %w", err)
			}
		}
		if statementTimeoutMS > 0 {
			if _, err := conn.Exec(ctx, fmt.Sprintf("SET statement_timeout = %d", statementTimeoutMS)); err != nil {
				return fmt.Errorf("kingbase: set statement_timeout: %w", err)
			}
		}
		return nil
	}
	...
	if params.Options.Tunnel == nil || !params.Options.Tunnel.Enabled() {
		if params.Options.Proxy != nil {
			...
			if derr != nil {
				return nil, fmt.Errorf("kingbase: proxy: %w", derr)
			}
