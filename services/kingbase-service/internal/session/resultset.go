...
func OpenPagedQuery(
	...
	if sess == nil {
		return nil, fmt.Errorf("kingbase: session required for paged query")
	}
	if pool == nil {
	...
	if releaseOwned != nil && !sess.IsAutoCommit() {
		cleanupFailed()
		return nil, fmt.Errorf("kingbase: cannot use a different database while auto-commit is off")
	}
...
func (s *Session) getResultSet(id string) (*ResultSet, error) {
	...
	if !ok {
		return nil, fmt.Errorf("kingbase: result set not found: %s", id)
	}
	return rs, nil
}
...
func (rs *ResultSet) readPage(limit int) (page [][]any, hasMore bool, truncated bool, err error) {
	...
	defer rs.mu.Unlock()
	if rs.closed {
		return nil, false, false, fmt.Errorf("kingbase: result set closed")
	}
	...
	for len(page) < limit {
		...
		if verr != nil {
			return nil, false, false, fmt.Errorf("kingbase: row values: %w", verr)
		}
	...
	if err := rs.rows.Err(); err != nil {
		return nil, false, false, fmt.Errorf("kingbase: rows: %w", err)
	}
