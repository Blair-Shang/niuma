...
/// default_dial_timeout 是 SFTP 建连默认超时（30 秒）。
const DEFAULT_DIAL_TIMEOUT_SECS: u64 = 30;

/// default_sftp_port 与 SSH 相同，默认 22。
...
/// ConnectOptions 与 Web connection_options JSON 对齐（认证字段同 SSH）。
...
pub enum ConnectError {
    #[error("sftp: connect {host}:{port}: {source}")]
    Dial {
        host: String,
        port: u16,
        source: russh::Error,
    },
    #[error("sftp: authentication failed for {user}@{host}")]
    AuthFailed { user: String, host: String },
    #[error("sftp: private key required")]
    PrivateKeyRequired,
    #[error("sftp: private key path required")]
    PrivateKeyPathRequired,
    #[error("sftp: unsupported auth type: {0}")]
    UnsupportedAuthType(String),
    #[error("sftp: host key rejected for {host}:{port} fingerprint={fingerprint} algo={algorithm} reason={reason}")]
    HostKeyRejected {
        host: String,
        port: u16,
        fingerprint: String,
    ...
    #[error("sftp: private key: {0}")]
    Key(#[from] russh_keys::Error),
}

/// SftpClientHandler 是 russh 客户端回调；可选对照 ~/.ssh/known_hosts 验证主机密钥。
...
/// connect_transport 建立已认证的 SSH 传输（不申请 shell）。
/// 返回 (会话句柄, 可选隧道守卫)；隧道守卫 drop 时自动关闭本地转发端口。
pub async fn connect_transport(
    params: &ConnectParams,
) -> Result<(client::Handle<SftpClientHandler>, Option<niuma_tunnel::TunnelGuard>), ConnectError> {
...
/// open_sftp_session 在已认证传输上打开 SFTP 子系统并完成握手。
/// 不申请 PTY / shell；账号仅有 SFTP 权限时也可成功。
...
fn host_key_rejected_error(host: &str, port: u16) -> ConnectError {
    let (fingerprint, algorithm, reason) = super::hostkey::peek_rejected(host, port)
        .unwrap_or_else(|| (String::new(), String::new(), "unknown".to_string()));
...
/// authenticate_password_or_kbdint 先试 password，失败再走 keyboard-interactive。
...
/// authenticate_keyboard_interactive 用已存口令回答 echo=false 的提示。
