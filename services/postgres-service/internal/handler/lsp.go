...
func (d *Dispatcher) ensureLSP() *sqllsp.Server {
	...
	srv := sqllsp.NewServer(d.pgParser, &postgresLSPCatalog{d: d}, d.lspConns, notify)
	srv.SourceName = "postgres-lsp"
...
func (d *Dispatcher) parserForSession(sessionID string) *postgresparser.Parser {
	mode := postgresparser.CompatPG
	if sessionID != "" {
		if s, err := d.sessions.Get(sessionID); err == nil && s != nil && s.Dialect != nil {
			mode = postgresparser.ParseCompat(s.Dialect.SQLCompatibility)
		}
	}
	return postgresparser.NewWithCompat(mode)
}
...
func (d *Dispatcher) lspLexicon(_ context.Context, r Request) Response {
	...
	var p *postgresparser.Parser
	switch {
	case strings.TrimSpace(params.SessionID) != "":
		p = d.parserForSession(params.SessionID)
	case strings.TrimSpace(params.Compat) != "":
		p = postgresparser.NewWithCompat(postgresparser.ParseCompat(params.Compat))
	default:
		p = postgresparser.New()
	}
...
// postgresLSPCatalog 进程内复用 catalog 查询逻辑。
type postgresLSPCatalog struct {
	d *Dispatcher
}

func (c *postgresLSPCatalog) ListSchemas(ctx context.Context, p sqllsp.CatalogParams) ([]sqllsp.SchemaHit, bool, error) {
...
func (c *postgresLSPCatalog) ListTables(ctx context.Context, p sqllsp.CatalogParams) ([]sqllsp.TableHit, bool, error) {
...
func (c *postgresLSPCatalog) ListColumns(ctx context.Context, p sqllsp.CatalogParams) ([]sqllsp.ColumnHit, bool, error) {
	schema := strings.TrimSpace(p.Schema)
	if schema == "" {
		schema = "public"
	}
	...
	if table == "" {
		return nil, false, fmt.Errorf("postgres: schema and table required")
	}
	db, _, release, err := c.resolve(ctx, p.SessionID, p.Database)
	if err != nil {
		return nil, false, err
...
func (c *postgresLSPCatalog) ListRoutines(ctx context.Context, p sqllsp.CatalogParams) ([]sqllsp.RoutineHit, bool, error) {
	schema := strings.TrimSpace(p.Schema)
	if schema == "" {
		schema = "public"
	}
...
func (c *postgresLSPCatalog) resolve(ctx context.Context, sessionID, database string) (*pgxpool.Pool, *session.Session, func(), error) {
