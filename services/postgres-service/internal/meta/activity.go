...
`).Scan(
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: instance overview: %w", err)
	}
	return out, nil
}
...
func ListActivity(ctx context.Context, pool *pgxpool.Pool) (*ActivityResult, error) {
	out, err := listActivity(ctx, pool, activitySQLWithQueryID)
...
// activitySQLWithQueryID：PostgreSQL 14+ 标准 query_id。
const activitySQLWithQueryID = `
SELECT a.pid::bigint,
...
func listActivity(ctx context.Context, pool *pgxpool.Pool, querySQL string) (*ActivityResult, error) {
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("postgres: activity rows: %w", err)
	}
	return out, nil
}
...
`)
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("postgres: scan lock: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("postgres: locks rows: %w", err)
	}
	return out, nil
}
...
`)
	if err != nil {
		return nil, fmt.Errorf("postgres: list locks: %w", err)
	}
	defer rows.Close()
...
func queryBlockingEdges(ctx context.Context, pool *pgxpool.Pool, querySQL string) ([]LockBlockingEdge, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&e.BlockedPID, &e.BlockingPID); err != nil {
			return nil, fmt.Errorf("postgres: scan blocking edge: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("postgres: blocking edges rows: %w", err)
	}
	return out, nil
}
...
func backendAction(ctx context.Context, pool *pgxpool.Pool, pid int64, fn string) (*BackendActionResult, error) {
	if pid <= 0 {
		return nil, fmt.Errorf("postgres: invalid backend pid")
	}
	if fn != "pg_cancel_backend" && fn != "pg_terminate_backend" {
		return nil, fmt.Errorf("postgres: unknown backend action")
	}
	var self int64
	if err := pool.QueryRow(ctx, `SELECT pg_backend_pid()`).Scan(&self); err != nil {
		return nil, fmt.Errorf("postgres: read backend pid: %w", err)
	}
	if pid == self {
		return nil, fmt.Errorf("postgres: cannot act on current backend")
	}

	// 优先 bigint：线程模型 pid 远超 int4；回退经 pg_stat_activity 取同类型 pid 再调用。
	ok, err := callBackendFn(ctx, pool, fn, pid)
	if err != nil {
		return nil, fmt.Errorf("postgres: backend action: %w", err)
	}
