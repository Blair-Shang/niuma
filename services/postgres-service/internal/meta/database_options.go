...
ORDER BY 1`
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: list encodings: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&enc); err != nil {
			return nil, fmt.Errorf("postgres: list encodings scan: %w", err)
		}
...
func scanStringList(ctx context.Context, pool *pgxpool.Pool, query string, args ...any) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: query options: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&value); err != nil {
			return nil, fmt.Errorf("postgres: scan options: %w", err)
		}
		value = strings.TrimSpace(value)
		if value == "" {
			continue
