...
func ListServerVariables(ctx context.Context, pool *pgxpool.Pool, like string) (*ServerKVResult, error) {
	if pool == nil {
		return nil, fmt.Errorf("postgres: server variables: nil pool")
	}
	like = clampLike(like)
...
`, like)
	if err != nil {
		return nil, fmt.Errorf("postgres: server variables: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&name, &value); err != nil {
			return nil, fmt.Errorf("postgres: scan server variables: %w", err)
		}
...
func ListServerStatus(ctx context.Context, pool *pgxpool.Pool, like string) (*ServerKVResult, error) {
	if pool == nil {
		return nil, fmt.Errorf("postgres: server status: nil pool")
	}
	like = clampLike(like)
...
func listDatabaseStatus(ctx context.Context, pool *pgxpool.Pool) ([]ServerKVItem, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: server status database: %w", err)
	}
	return items, nil
}
