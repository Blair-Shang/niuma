...
ORDER BY a.attnum`
	...
	if err != nil {
		if orderedErr != nil {
			return nil, fmt.Errorf("postgres: list primary key: %w", orderedErr)
		}
		return nil, fmt.Errorf("postgres: list primary key: %w", err)
	}
	defer rows.Close()
...
func scanPrimaryKeyColumnNames(rows pgx.Rows) ([]string, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&name); err != nil {
			return nil, fmt.Errorf("postgres: primary key scan: %w", err)
		}
