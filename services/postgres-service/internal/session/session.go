...
import (
	...
	"niuma/services/postgres-service/internal/dialect"
)
...
// Session 持有一条 PostgreSQL 连接池、在途查询取消句柄与打开的结果游标。
type Session struct {
	ID         string
	Pool       *pgxpool.Pool
	Params     ConnectParams
	TunnelStop func()
	// Dialect 连接时探测的方言能力集（对齐 Navicat/DBeaver 产品类型行为）。
	...
	// 事务（对齐 mysql-service：关自动提交时钉住物理连接）。
	autoCommit bool // 默认按 TxStateSnapshot：无钉连接即视为 true
	inTx       bool
	txConn     *pgxpool.Conn
	txBusy     bool
}
...
func (s *Session) CancelQuery(requestID string) int {
	n := 0
	if requestID != "" {
		// 先关同 request 的游标，再 cancel context。
		s.mu.Lock()
