...
func (s *Session) acquireExecConn(ctx context.Context, pool *pgxpool.Pool) (conn *pgxpool.Conn, sessionOwned bool, release func(), err error) {
	...
	if useTx {
		if s.txBusy {
			s.mu.Unlock()
			return nil, false, nil, fmt.Errorf("postgres: transaction connection busy (close open result cursor first)")
		}
		conn = s.txConn
		s.txBusy = true
		s.mu.Unlock()
	...
	if pool == nil {
		return nil, false, nil, fmt.Errorf("postgres: pool required")
	}
	conn, err = pool.Acquire(ctx)
	if err != nil {
		return nil, false, nil, fmt.Errorf("postgres: acquire: %w", err)
	}
...
func (s *Session) SetAutoCommit(ctx context.Context, enabled bool) (TxState, error) {
	if s.Pool == nil {
		return TxState{}, fmt.Errorf("postgres: session closed")
	}
	...
	s.mu.Unlock()
	if busy {
		return TxState{}, fmt.Errorf("postgres: transaction connection busy")
	}
	...
	if conn != nil {
		if inTx {
			if _, err := conn.Exec(ctx, "ROLLBACK"); err != nil {
				return TxState{}, fmt.Errorf("postgres: rollback before auto-commit: %w", err)
			}
		}
		conn.Release()
	}
...
func (s *Session) ensureAutoCommitOff(ctx context.Context) (TxState, error) {
	...
	conn, err := pool.Acquire(ctx)
	if err != nil {
		return TxState{}, fmt.Errorf("postgres: acquire tx conn: %w", err)
	}
...
func (s *Session) Commit(ctx context.Context) (TxState, error) {
	...
	s.mu.Unlock()
	if auto {
		return TxState{AutoCommit: true}, fmt.Errorf("postgres: auto-commit is on; nothing to commit")
	}
	if busy {
		return TxState{}, fmt.Errorf("postgres: transaction connection busy")
	}
	if _, err := conn.Exec(ctx, "COMMIT"); err != nil {
		return TxState{}, fmt.Errorf("postgres: commit: %w", err)
	}
	s.mu.Lock()
	s.inTx = false
	s.mu.Unlock()
...
func (s *Session) Rollback(ctx context.Context) (TxState, error) {
	...
	s.mu.Unlock()
	if auto {
		return TxState{AutoCommit: true}, fmt.Errorf("postgres: auto-commit is on; nothing to rollback")
	}
	if busy {
		return TxState{}, fmt.Errorf("postgres: transaction connection busy")
	}
	if _, err := conn.Exec(ctx, "ROLLBACK"); err != nil {
		return TxState{}, fmt.Errorf("postgres: rollback: %w", err)
	}
	s.mu.Lock()
	s.inTx = false
	s.mu.Unlock()
