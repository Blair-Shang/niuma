...
type ResultSet struct {
	...
	// sessionOwned：事务钉连接，关闭游标时不得 Release 回池，只清 txBusy。
	sessionOwned bool
	releaseBusy  func()
}
...
func OpenPagedQuery(
	...
	if sess == nil {
		return nil, fmt.Errorf("postgres: session required for paged query")
	}
	if pool == nil {
	...
	if sqlText == "" {
		return nil, fmt.Errorf("postgres: sql required")
	}
	...
	// 非 Auto-commit 时禁止跨库短连（事务钉在会话默认库连接上）。
	if releaseOwned != nil && !sess.IsAutoCommit() {
		cleanupFailed()
		return nil, fmt.Errorf("postgres: cannot use a different database while auto-commit is off")
	}

	conn, sessionOwned, releaseBusy, err := sess.acquireExecConn(rsCtx, pool)
	if err != nil {
		cleanupFailed()
		return nil, err
	}
	if err := sess.beginTxIfNeeded(rsCtx, conn); err != nil {
		if !sessionOwned {
			conn.Release()
		} else if releaseBusy != nil {
			releaseBusy()
		}
		cleanupFailed()
		return nil, err
...
func (s *Session) getResultSet(id string) (*ResultSet, error) {
	...
	if !ok {
		return nil, fmt.Errorf("postgres: result set not found: %s", id)
	}
	return rs, nil
}
...
func (rs *ResultSet) readPage(limit int) (page [][]any, hasMore bool, truncated bool, err error) {
	...
	defer rs.mu.Unlock()
	if rs.closed {
		return nil, false, false, fmt.Errorf("postgres: result set closed")
	}
	...
	for len(page) < limit {
		...
		if verr != nil {
			return nil, false, false, fmt.Errorf("postgres: row values: %w", verr)
		}
	...
	if err := rs.rows.Err(); err != nil {
		return nil, false, false, fmt.Errorf("postgres: rows: %w", err)
	}
...
func (rs *ResultSet) finishLocked() {
	...
	if rs.conn != nil {
		if rs.sessionOwned {
			// 事务钉连接：归还忙标记，不 Release 回池。
			rs.conn = nil
			if rs.releaseBusy != nil {
				rs.releaseBusy()
				rs.releaseBusy = nil
			}
		} else {
