...
LIMIT $2`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&name); err != nil {
			return nil, fmt.Errorf("postgres: list databases scan: %w", err)
		}
...
// 官方 PostgreSQL 系统 schema（不含金仓 / openGauss 厂商包）。
...
LIMIT $4`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&name, &kind); err != nil {
			return nil, fmt.Errorf("postgres: list tables scan: %w", err)
		}
...
LIMIT $4`
	...
	rows, err := pool.Query(ctx, query, args...)
	if err != nil {
		return nil, fmt.Errorf("postgres: list routines: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&info.OID, &info.Name, &info.Kind, &info.Args); err != nil {
			return nil, fmt.Errorf("postgres: list routines scan: %w", err)
		}
		out = append(out, info)
	}
	if err := rows.Err(); err != nil {
...
LIMIT $3`
	...
	rows, err := pool.Query(ctx, query, args...)
	if err != nil {
		return nil, fmt.Errorf("postgres: list sequences: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		var name string
		if err := rows.Scan(&name); err != nil {
			return nil, fmt.Errorf("postgres: list sequences scan: %w", err)
		}
...
LIMIT $3`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&info.OID, &info.Name, &info.TableName); err != nil {
			return nil, fmt.Errorf("postgres: list triggers scan: %w", err)
		}
		out = append(out, info)
	}
	if err := rows.Err(); err != nil {
