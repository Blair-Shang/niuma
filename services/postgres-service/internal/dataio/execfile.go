...
func execSqlFile(
	...
	if err != nil {
		return fmt.Errorf("postgres: open sql file: %w", err)
	}
	defer f.Close()
	...
	handleErr := func(stmtNo int, kind string, err error) error {
		if !opts.ContinueOnError {
			if kind == "copy" {
				return fmt.Errorf("postgres: exec sql file copy near statement %d: %w", stmtNo, err)
			}
			return fmt.Errorf("postgres: exec sql file near statement %d: %w", stmtNo, err)
		}
	...
	if failed > 0 {
		...
		return fmt.Errorf("postgres: completed with %d error(s), %d succeeded", failed, executed)
	}
