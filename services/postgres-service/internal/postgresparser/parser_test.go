...
func TestKeywordsNonEmpty(t *testing.T) {
	...
	if len(p.Keywords()) < 20 {
		t.Fatal("expected postgres keywords")
	}
}
...
func TestClassify(t *testing.T) {
	...
	for _, c := range cases {
		got := postgresparser.Classify(c.sql).String()
		if got != c.kind {
...
func TestCompletionContextSelectColumn(t *testing.T) {
	...
	cc := postgresparser.New().CompletionContext(sql, sqllsp.Position{Character: len(sql)})
...
func TestCompletionSnippetsNoDelimiter(t *testing.T) {
	...
	for _, sn := range cc.Snippets {
		if strings.Contains(strings.ToUpper(sn.InsertText), "DELIMITER") {
			t.Fatalf("postgres snippet must not use DELIMITER: %s", sn.Label)
		}
		if strings.Contains(sn.InsertText, "`") {
			t.Fatalf("postgres snippet must use double-quote ident, got backtick in %s", sn.Label)
		}
	}
}
...
func TestWorkASTTables(t *testing.T) {
	...
	ast := postgresparser.ParseWorkAST(sql, sqllsp.Position{Character: len(sql)})
	if len(ast.Tables) < 2 {
...
func TestCompatOracleLimitHint(t *testing.T) {
	p := postgresparser.NewWithCompat(postgresparser.CompatOracle)
...
func TestCompatAutoNoMysqlOracleUnion(t *testing.T) {
	p := postgresparser.NewWithCompat(postgresparser.CompatAuto)
	kws := p.Keywords()
...
func TestCompatMysqlBuiltinsExclusive(t *testing.T) {
	p := postgresparser.NewWithCompat(postgresparser.CompatMysql)
	if p.BuiltinSignature("IFNULL") == nil {
...
func TestEmptyDiagnostics(t *testing.T) {
	if got := postgresparser.New().Diagnostics("u", " \n\t"); len(got) != 0 {
		t.Fatalf("empty SQL diagnostics = %#v", got)
	}
}

func TestPGKeywordIsolation(t *testing.T) {
	for _, keyword := range postgresparser.NewWithCompat(postgresparser.CompatPG).Keywords() {
...
func TestDollarQuotedSplit(t *testing.T) {
	...
	spans := postgresparser.New().SplitStatements(sql)
	if len(spans) != 2 {
