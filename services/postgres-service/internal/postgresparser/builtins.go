...
// pgNativeBuiltins PostgreSQL 常用函数。
func pgNativeBuiltins() []builtinFn {
	return []builtinFn{
...
func builtinsForCompat(mode CompatMode) []builtinFn {
	out := append([]builtinFn(nil), pgNativeBuiltins()...)
	switch mode {
	case CompatOracle:
...
// postgresBuiltinFunctionNames 内置函数名列表（去重，供 Monarch / lexicon）。
func postgresBuiltinFunctionNames(mode CompatMode) []string {
...
func postgresBuiltinFunctions(mode CompatMode) []sqllsp.CompletionItem {
	defs := builtinsForCompat(mode)
...
// QuoteIdent 实现 sqllsp.IdentifierQuoter（PostgreSQL 双引号）。
