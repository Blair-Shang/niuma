...
// Parser 实现 sqllsp.DialectParser（PostgreSQL）及可选扩展接口。
type Parser struct {
	keywords []string
	compat   CompatMode
}

// New 创建默认 PostgreSQL 解析器。
func New() *Parser {
...
func (p *Parser) Functions() []string {
	return postgresBuiltinFunctionNames(p.compat)
}
...
func (p *Parser) CompletionContext(text string, pos sqllsp.Position) sqllsp.CompletionContext {
	...
	cc.Snippets = postgresCreateSnippets()
	cc.Functions = postgresBuiltinFunctions(p.compat)
...
func postgresCreateSnippets() []sqllsp.CompletionItem {
