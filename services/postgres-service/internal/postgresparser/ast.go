...
func ParseWorkAST(text string, pos sqllsp.Position) WorkAST {
	...
	start, stmt := currentStatementSpanKingbase(text, offset)
	ast := WorkAST{
		Kind:      Classify(stmt),
		StmtStart: start,
		StmtText:  stmt,
		Tables:    extractTableRefsKingbase(stmt, start),
	}
	...
	if beginAt >= 0 {
		...
		if end, ok := findMatchingEnd(text, bodyStart, 1); ok {
			...
			for _, st := range splitTopLevelStatements(body) {
				...
				refs := extractTableRefsKingbase(st.text, base+bodyStart+st.start)
...
// currentStatementSpanKingbase 感知 `/` 终止符的语句切片。
func currentStatementSpanKingbase(text string, offset int) (start int, stmt string) {
...
func extractTableRefsKingbase(stmt string, base int) []sqllsp.TableRef {
